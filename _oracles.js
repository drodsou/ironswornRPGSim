export var oracles = {
      ironsworn: {
        plot: {
          oracles: {
            action: {
              name: 'action',
              roll_id: 'Action',
              dice: 'd100',
              columns: '3',
              rows: [
                {
                  type: 'Total',
                  roll: '1',
                  result: "Scheme"
                },
                {
                  type: 'Total',
                  roll: '2',
                  result: "Clash"
                },
                {
                  type: 'Total',
                  roll: '3',
                  result: "Weaken"
                },
                {
                  type: 'Total',
                  roll: '4',
                  result: "Initiate"
                },
                {
                  type: 'Total',
                  roll: '5',
                  result: "Create"
                },
                {
                  type: 'Total',
                  roll: '6',
                  result: "Swear"
                },
                {
                  type: 'Total',
                  roll: '7',
                  result: "Avenge"
                },
                {
                  type: 'Total',
                  roll: '8',
                  result: "Guard"
                },
                {
                  type: 'Total',
                  roll: '9',
                  result: "Defeat"
                },
                {
                  type: 'Total',
                  roll: '10',
                  result: "Control"
                },
                {
                  type: 'Total',
                  roll: '11',
                  result: "Break"
                },
                {
                  type: 'Total',
                  roll: '12',
                  result: "Risk"
                },
                {
                  type: 'Total',
                  roll: '13',
                  result: "Surrender"
                },
                {
                  type: 'Total',
                  roll: '14',
                  result: "Inspect"
                },
                {
                  type: 'Total',
                  roll: '15',
                  result: "Raid"
                },
                {
                  type: 'Total',
                  roll: '16',
                  result: "Evade"
                },
                {
                  type: 'Total',
                  roll: '17',
                  result: "Assault"
                },
                {
                  type: 'Total',
                  roll: '18',
                  result: "Deflect"
                },
                {
                  type: 'Total',
                  roll: '19',
                  result: "Threaten"
                },
                {
                  type: 'Total',
                  roll: '20',
                  result: "Attack"
                },
                {
                  type: 'Total',
                  roll: '21',
                  result: "Leave"
                },
                {
                  type: 'Total',
                  roll: '22',
                  result: "Preserve"
                },
                {
                  type: 'Total',
                  roll: '23',
                  result: "Manipulate"
                },
                {
                  type: 'Total',
                  roll: '24',
                  result: "Remove"
                },
                {
                  type: 'Total',
                  roll: '25',
                  result: "Eliminate"
                },
                {
                  type: 'Total',
                  roll: '26',
                  result: "Withdraw"
                },
                {
                  type: 'Total',
                  roll: '27',
                  result: "Abandon"
                },
                {
                  type: 'Total',
                  roll: '28',
                  result: "Investigate"
                },
                {
                  type: 'Total',
                  roll: '29',
                  result: "Hold"
                },
                {
                  type: 'Total',
                  roll: '30',
                  result: "Focus"
                },
                {
                  type: 'Total',
                  roll: '31',
                  result: "Uncover"
                },
                {
                  type: 'Total',
                  roll: '32',
                  result: "Breach"
                },
                {
                  type: 'Total',
                  roll: '33',
                  result: "Aid"
                },
                {
                  type: 'Total',
                  roll: '34',
                  result: "Uphold"
                },
                {
                  type: 'Total',
                  roll: '35',
                  result: "Falter"
                },
                {
                  type: 'Total',
                  roll: '36',
                  result: "Suppress"
                },
                {
                  type: 'Total',
                  roll: '37',
                  result: "Hunt"
                },
                {
                  type: 'Total',
                  roll: '38',
                  result: "Share"
                },
                {
                  type: 'Total',
                  roll: '39',
                  result: "Destroy"
                },
                {
                  type: 'Total',
                  roll: '40',
                  result: "Avoid"
                },
                {
                  type: 'Total',
                  roll: '41',
                  result: "Reject"
                },
                {
                  type: 'Total',
                  roll: '42',
                  result: "Demand"
                },
                {
                  type: 'Total',
                  roll: '43',
                  result: "Explore"
                },
                {
                  type: 'Total',
                  roll: '44',
                  result: "Bolster"
                },
                {
                  type: 'Total',
                  roll: '45',
                  result: "Seize"
                },
                {
                  type: 'Total',
                  roll: '46',
                  result: "Mourn"
                },
                {
                  type: 'Total',
                  roll: '47',
                  result: "Reveal"
                },
                {
                  type: 'Total',
                  roll: '48',
                  result: "Gather"
                },
                {
                  type: 'Total',
                  roll: '49',
                  result: "Defy"
                },
                {
                  type: 'Total',
                  roll: '50',
                  result: "Transform"
                },
                {
                  type: 'Total',
                  roll: '51',
                  result: "Persevere"
                },
                {
                  type: 'Total',
                  roll: '52',
                  result: "Serve"
                },
                {
                  type: 'Total',
                  roll: '53',
                  result: "Begin"
                },
                {
                  type: 'Total',
                  roll: '54',
                  result: "Move"
                },
                {
                  type: 'Total',
                  roll: '55',
                  result: "Coordinate"
                },
                {
                  type: 'Total',
                  roll: '56',
                  result: "Resist"
                },
                {
                  type: 'Total',
                  roll: '57',
                  result: "Await"
                },
                {
                  type: 'Total',
                  roll: '58',
                  result: "Impress"
                },
                {
                  type: 'Total',
                  roll: '59',
                  result: "Take"
                },
                {
                  type: 'Total',
                  roll: '60',
                  result: "Oppose"
                },
                {
                  type: 'Total',
                  roll: '61',
                  result: "Capture"
                },
                {
                  type: 'Total',
                  roll: '62',
                  result: "Overwhelm"
                },
                {
                  type: 'Total',
                  roll: '63',
                  result: "Challenge"
                },
                {
                  type: 'Total',
                  roll: '64',
                  result: "Acquire"
                },
                {
                  type: 'Total',
                  roll: '65',
                  result: "Protect"
                },
                {
                  type: 'Total',
                  roll: '66',
                  result: "Finish"
                },
                {
                  type: 'Total',
                  roll: '67',
                  result: "Strengthen"
                },
                {
                  type: 'Total',
                  roll: '68',
                  result: "Restore"
                },
                {
                  type: 'Total',
                  roll: '69',
                  result: "Advance"
                },
                {
                  type: 'Total',
                  roll: '70',
                  result: "Command"
                },
                {
                  type: 'Total',
                  roll: '71',
                  result: "Refuse"
                },
                {
                  type: 'Total',
                  roll: '72',
                  result: "Find"
                },
                {
                  type: 'Total',
                  roll: '73',
                  result: "Deliver"
                },
                {
                  type: 'Total',
                  roll: '74',
                  result: "Hide"
                },
                {
                  type: 'Total',
                  roll: '75',
                  result: "Fortify"
                },
                {
                  type: 'Total',
                  roll: '76',
                  result: "Betray"
                },
                {
                  type: 'Total',
                  roll: '77',
                  result: "Secure"
                },
                {
                  type: 'Total',
                  roll: '78',
                  result: "Arrive"
                },
                {
                  type: 'Total',
                  roll: '79',
                  result: "Affect"
                },
                {
                  type: 'Total',
                  roll: '80',
                  result: "Change"
                },
                {
                  type: 'Total',
                  roll: '81',
                  result: "Defend"
                },
                {
                  type: 'Total',
                  roll: '82',
                  result: "Debate"
                },
                {
                  type: 'Total',
                  roll: '83',
                  result: "Support"
                },
                {
                  type: 'Total',
                  roll: '84',
                  result: "Follow"
                },
                {
                  type: 'Total',
                  roll: '85',
                  result: "Construct"
                },
                {
                  type: 'Total',
                  roll: '86',
                  result: "Locate"
                },
                {
                  type: 'Total',
                  roll: '87',
                  result: "Endure"
                },
                {
                  type: 'Total',
                  roll: '88',
                  result: "Release"
                },
                {
                  type: 'Total',
                  roll: '89',
                  result: "Lose"
                },
                {
                  type: 'Total',
                  roll: '90',
                  result: "Reduce"
                },
                {
                  type: 'Total',
                  roll: '91',
                  result: "Escalate"
                },
                {
                  type: 'Total',
                  roll: '92',
                  result: "Distract"
                },
                {
                  type: 'Total',
                  roll: '93',
                  result: "Journey"
                },
                {
                  type: 'Total',
                  roll: '94',
                  result: "Escort"
                },
                {
                  type: 'Total',
                  roll: '95',
                  result: "Learn"
                },
                {
                  type: 'Total',
                  roll: '96',
                  result: "Communicate"
                },
                {
                  type: 'Total',
                  roll: '97',
                  result: "Depart"
                },
                {
                  type: 'Total',
                  roll: '98',
                  result: "Search"
                },
                {
                  type: 'Total',
                  roll: '99',
                  result: "Charge"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Summon"
                }
              ]
            },
            theme: {
              name: 'theme',
              roll_id: 'Theme',
              dice: 'd100',
              columns: '3',
              rows: [
                {
                  type: 'Total',
                  roll: '1',
                  result: "Risk"
                },
                {
                  type: 'Total',
                  roll: '2',
                  result: "Ability"
                },
                {
                  type: 'Total',
                  roll: '3',
                  result: "Price"
                },
                {
                  type: 'Total',
                  roll: '4',
                  result: "Ally"
                },
                {
                  type: 'Total',
                  roll: '5',
                  result: "Battle"
                },
                {
                  type: 'Total',
                  roll: '6',
                  result: "Safety"
                },
                {
                  type: 'Total',
                  roll: '7',
                  result: "Survival"
                },
                {
                  type: 'Total',
                  roll: '8',
                  result: "Weapon"
                },
                {
                  type: 'Total',
                  roll: '9',
                  result: "Wound"
                },
                {
                  type: 'Total',
                  roll: '10',
                  result: "Shelter"
                },
                {
                  type: 'Total',
                  roll: '11',
                  result: "Leader"
                },
                {
                  type: 'Total',
                  roll: '12',
                  result: "Fear"
                },
                {
                  type: 'Total',
                  roll: '13',
                  result: "Time"
                },
                {
                  type: 'Total',
                  roll: '14',
                  result: "Duty"
                },
                {
                  type: 'Total',
                  roll: '15',
                  result: "Secret"
                },
                {
                  type: 'Total',
                  roll: '16',
                  result: "Innocence"
                },
                {
                  type: 'Total',
                  roll: '17',
                  result: "Renown"
                },
                {
                  type: 'Total',
                  roll: '18',
                  result: "Direction"
                },
                {
                  type: 'Total',
                  roll: '19',
                  result: "Death"
                },
                {
                  type: 'Total',
                  roll: '20',
                  result: "Honor"
                },
                {
                  type: 'Total',
                  roll: '21',
                  result: "Labor"
                },
                {
                  type: 'Total',
                  roll: '22',
                  result: "Solution"
                },
                {
                  type: 'Total',
                  roll: '23',
                  result: "Tool"
                },
                {
                  type: 'Total',
                  roll: '24',
                  result: "Balance"
                },
                {
                  type: 'Total',
                  roll: '25',
                  result: "Love"
                },
                {
                  type: 'Total',
                  roll: '26',
                  result: "Barrier"
                },
                {
                  type: 'Total',
                  roll: '27',
                  result: "Creation"
                },
                {
                  type: 'Total',
                  roll: '28',
                  result: "Decay"
                },
                {
                  type: 'Total',
                  roll: '29',
                  result: "Trade"
                },
                {
                  type: 'Total',
                  roll: '30',
                  result: "Bond"
                },
                {
                  type: 'Total',
                  roll: '31',
                  result: "Hope"
                },
                {
                  type: 'Total',
                  roll: '32',
                  result: "Superstition"
                },
                {
                  type: 'Total',
                  roll: '33',
                  result: "Peace"
                },
                {
                  type: 'Total',
                  roll: '34',
                  result: "Deception"
                },
                {
                  type: 'Total',
                  roll: '35',
                  result: "History"
                },
                {
                  type: 'Total',
                  roll: '36',
                  result: "World"
                },
                {
                  type: 'Total',
                  roll: '37',
                  result: "Vow"
                },
                {
                  type: 'Total',
                  roll: '38',
                  result: "Protection"
                },
                {
                  type: 'Total',
                  roll: '39',
                  result: "Nature"
                },
                {
                  type: 'Total',
                  roll: '40',
                  result: "Opinion"
                },
                {
                  type: 'Total',
                  roll: '41',
                  result: "Burden"
                },
                {
                  type: 'Total',
                  roll: '42',
                  result: "Vengeance"
                },
                {
                  type: 'Total',
                  roll: '43',
                  result: "Opportunity"
                },
                {
                  type: 'Total',
                  roll: '44',
                  result: "Faction"
                },
                {
                  type: 'Total',
                  roll: '45',
                  result: "Danger"
                },
                {
                  type: 'Total',
                  roll: '46',
                  result: "Corruption"
                },
                {
                  type: 'Total',
                  roll: '47',
                  result: "Freedom"
                },
                {
                  type: 'Total',
                  roll: '48',
                  result: "Debt"
                },
                {
                  type: 'Total',
                  roll: '49',
                  result: "Hate"
                },
                {
                  type: 'Total',
                  roll: '50',
                  result: "Possession"
                },
                {
                  type: 'Total',
                  roll: '51',
                  result: "Stranger"
                },
                {
                  type: 'Total',
                  roll: '52',
                  result: "Passage"
                },
                {
                  type: 'Total',
                  roll: '53',
                  result: "Land"
                },
                {
                  type: 'Total',
                  roll: '54',
                  result: "Creature"
                },
                {
                  type: 'Total',
                  roll: '55',
                  result: "Disease"
                },
                {
                  type: 'Total',
                  roll: '56',
                  result: "Advantage"
                },
                {
                  type: 'Total',
                  roll: '57',
                  result: "Blood"
                },
                {
                  type: 'Total',
                  roll: '58',
                  result: "Language"
                },
                {
                  type: 'Total',
                  roll: '59',
                  result: "Rumor"
                },
                {
                  type: 'Total',
                  roll: '60',
                  result: "Weakness"
                },
                {
                  type: 'Total',
                  roll: '61',
                  result: "Greed"
                },
                {
                  type: 'Total',
                  roll: '62',
                  result: "Family"
                },
                {
                  type: 'Total',
                  roll: '63',
                  result: "Resource"
                },
                {
                  type: 'Total',
                  roll: '64',
                  result: "Structure"
                },
                {
                  type: 'Total',
                  roll: '65',
                  result: "Dream"
                },
                {
                  type: 'Total',
                  roll: '66',
                  result: "Community"
                },
                {
                  type: 'Total',
                  roll: '67',
                  result: "War"
                },
                {
                  type: 'Total',
                  roll: '68',
                  result: "Portent"
                },
                {
                  type: 'Total',
                  roll: '69',
                  result: "Prize"
                },
                {
                  type: 'Total',
                  roll: '70',
                  result: "Destiny"
                },
                {
                  type: 'Total',
                  roll: '71',
                  result: "Momentum"
                },
                {
                  type: 'Total',
                  roll: '72',
                  result: "Power"
                },
                {
                  type: 'Total',
                  roll: '73',
                  result: "Memory"
                },
                {
                  type: 'Total',
                  roll: '74',
                  result: "Ruin"
                },
                {
                  type: 'Total',
                  roll: '75',
                  result: "Mysticism"
                },
                {
                  type: 'Total',
                  roll: '76',
                  result: "Rival"
                },
                {
                  type: 'Total',
                  roll: '77',
                  result: "Problem"
                },
                {
                  type: 'Total',
                  roll: '78',
                  result: "Idea"
                },
                {
                  type: 'Total',
                  roll: '79',
                  result: "Revenge"
                },
                {
                  type: 'Total',
                  roll: '80',
                  result: "Health"
                },
                {
                  type: 'Total',
                  roll: '81',
                  result: "Fellowship"
                },
                {
                  type: 'Total',
                  roll: '82',
                  result: "Enemy"
                },
                {
                  type: 'Total',
                  roll: '83',
                  result: "Religion"
                },
                {
                  type: 'Total',
                  roll: '84',
                  result: "Spirit"
                },
                {
                  type: 'Total',
                  roll: '85',
                  result: "Fame"
                },
                {
                  type: 'Total',
                  roll: '86',
                  result: "Desolation"
                },
                {
                  type: 'Total',
                  roll: '87',
                  result: "Strength"
                },
                {
                  type: 'Total',
                  roll: '88',
                  result: "Knowledge"
                },
                {
                  type: 'Total',
                  roll: '89',
                  result: "Truth"
                },
                {
                  type: 'Total',
                  roll: '90',
                  result: "Quest"
                },
                {
                  type: 'Total',
                  roll: '91',
                  result: "Pride"
                },
                {
                  type: 'Total',
                  roll: '92',
                  result: "Loss"
                },
                {
                  type: 'Total',
                  roll: '93',
                  result: "Law"
                },
                {
                  type: 'Total',
                  roll: '94',
                  result: "Path"
                },
                {
                  type: 'Total',
                  roll: '95',
                  result: "Warning"
                },
                {
                  type: 'Total',
                  roll: '96',
                  result: "Relationship"
                },
                {
                  type: 'Total',
                  roll: '97',
                  result: "Wealth"
                },
                {
                  type: 'Total',
                  roll: '98',
                  result: "Home"
                },
                {
                  type: 'Total',
                  roll: '99',
                  result: "Strategy"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Supply"
                }
              ]
            },
            combat_actions: {
              name: 'combat_actions',
              roll_id: 'CombatActions',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-3',
                  result: "Compel a surrender"
                },
                {
                  type: 'Between',
                  roll: '4-6',
                  result: "Coordinate with allies"
                },
                {
                  type: 'Between',
                  roll: '7-9',
                  result: "Gather reinforcements"
                },
                {
                  type: 'Between',
                  roll: '10-13',
                  result: "Seize something or someone"
                },
                {
                  type: 'Between',
                  roll: '14-17',
                  result: "Provoke a reckless response"
                },
                {
                  type: 'Between',
                  roll: '18-21',
                  result: "Intimidate or frighten"
                },
                {
                  type: 'Between',
                  roll: '22-25',
                  result: "Reveal a surprising truth"
                },
                {
                  type: 'Between',
                  roll: '26-29',
                  result: "Shift focus to someone or something else"
                },
                {
                  type: 'Between',
                  roll: '30-33',
                  result: "Destroy something, or render it useless"
                },
                {
                  type: 'Between',
                  roll: '34-39',
                  result: "Take a decisive action"
                },
                {
                  type: 'Between',
                  roll: '40-45',
                  result: "Reinforce defenses"
                },
                {
                  type: 'Between',
                  roll: '46-52',
                  result: "Ready an action"
                },
                {
                  type: 'Between',
                  roll: '53-60',
                  result: "Use the terrain to gain advantage"
                },
                {
                  type: 'Between',
                  roll: '61-68',
                  result: "Leverage the advantage of a weapon or ability"
                },
                {
                  type: 'Between',
                  roll: '69-78',
                  result: "Create an opportunity"
                },
                {
                  type: 'Between',
                  roll: '79-89',
                  result: "Attack with precision"
                },
                {
                  type: 'Between',
                  roll: '90-99',
                  result: "Attack with power"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Take a completely unexpected action"
                }
              ]
            },
            settlement_trouble: {
              name: 'settlement_trouble',
              roll_id: 'SettlementTrouble',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Outsiders rejected"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Dangerous discovery"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Dreadful omens"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Natural disaster"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Old wounds reopened"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Important object is lost"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Someone is captured"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Mysterious phenomenon"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Revolt against a leader"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Vengeful outcast"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Rival settlement"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Nature strikes back"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Someone is missing"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Production halts"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Mysterious murders"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Debt comes due"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Unjust leadership"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Disastrous accident"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "In league with the enemy"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Raiders prey on the weak"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Cursed past"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "An innocent is accused"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Corrupted by dark magic"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Isolated by brutal weather"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Provisions are scarce"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Sickness run amok"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Allies become enemies"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Attack is imminent"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Lost caravan"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Dark secret revealed"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Urgent expedition"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "A leader falls"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Families in conflict"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Incompetent leadership"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Reckless warmongering"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Beast on the hunt"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Betrayed from within"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Broken truce"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Wrathful haunt"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Conflict with firstborn"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Trade route blocked"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "In the crossfire"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Stranger causes discord"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Important event threatened"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Dangerous tradition"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Roll twice"
                }
              ]
            },
            major_plot_twist: {
              name: 'major_plot_twist',
              roll_id: 'MajorPlotTwist',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-5',
                  result: "It was all a diversion."
                },
                {
                  type: 'Between',
                  roll: '6-10',
                  result: "A dark secret is revealed."
                },
                {
                  type: 'Between',
                  roll: '11-15',
                  result: "A trap is sprung."
                },
                {
                  type: 'Between',
                  roll: '16-20',
                  result: "An assumption is revealed to be false."
                },
                {
                  type: 'Between',
                  roll: '21-25',
                  result: "A secret alliance is revealed."
                },
                {
                  type: 'Between',
                  roll: '26-30',
                  result: "Your actions benefit an enemy."
                },
                {
                  type: 'Between',
                  roll: '31-35',
                  result: "Someone returns unexpectedly."
                },
                {
                  type: 'Between',
                  roll: '36-40',
                  result: "A more dangerous foe is revealed."
                },
                {
                  type: 'Between',
                  roll: '41-45',
                  result: "You and an enemy share a common goal."
                },
                {
                  type: 'Between',
                  roll: '46-50',
                  result: "A true identity is revealed."
                },
                {
                  type: 'Between',
                  roll: '51-55',
                  result: "You are betrayed by someone who was trusted."
                },
                {
                  type: 'Between',
                  roll: '56-60',
                  result: "You are too late."
                },
                {
                  type: 'Between',
                  roll: '61-65',
                  result: "The true enemy is revealed."
                },
                {
                  type: 'Between',
                  roll: '66-70',
                  result: "The enemy gains new allies."
                },
                {
                  type: 'Between',
                  roll: '71-75',
                  result: "A new danger appears."
                },
                {
                  type: 'Between',
                  roll: '76-80',
                  result: "Someone or something goes missing."
                },
                {
                  type: 'Between',
                  roll: '81-85',
                  result: "The truth of a relationship is revealed."
                },
                {
                  type: 'Between',
                  roll: '86-90',
                  result: "Two seemingly unrelated situations are shown to be connected."
                },
                {
                  type: 'Between',
                  roll: '91-95',
                  result: "Unexpected powers or abilities are revealed."
                },
                {
                  type: 'Between',
                  roll: '96-100',
                  result: "Roll twice more on this table. Both results occur. If they are the same result, make it more dramatic."
                }
              ]
            },
            mystic_backlash: {
              name: 'mystic_backlash',
              roll_id: 'MysticBacklash',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-4',
                  result: "Your ritual has the opposite affect."
                },
                {
                  type: 'Between',
                  roll: '5-8',
                  result: "You are sapped of strength."
                },
                {
                  type: 'Between',
                  roll: '9-12',
                  result: "Your friend, ally, or companion is adversely affected."
                },
                {
                  type: 'Between',
                  roll: '13-16',
                  result: "You destroy an important object."
                },
                {
                  type: 'Between',
                  roll: '17-20',
                  result: "You inadvertently summon a horror."
                },
                {
                  type: 'Between',
                  roll: '21-24',
                  result: "You collapse, and drift into a troubled sleep."
                },
                {
                  type: 'Between',
                  roll: '25-28',
                  result: "You undergo a physical torment which leaves its mark upon you."
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "You hear ghostly voices whispering of dark portents."
                },
                {
                  type: 'Between',
                  roll: '33-36',
                  result: "You are lost in shadow, and find yourself in another place without memory of how you got there."
                },
                {
                  type: 'Between',
                  roll: '37-40',
                  result: "You alert someone or something to your presence."
                },
                {
                  type: 'Between',
                  roll: '41-44',
                  result: "You are not yourself, and act against a friend, ally, or companion."
                },
                {
                  type: 'Between',
                  roll: '45-48',
                  result: "You affect or damage your surroundings, causing a disturbance or potential harm."
                },
                {
                  type: 'Between',
                  roll: '49-52',
                  result: "You waste resources."
                },
                {
                  type: 'Between',
                  roll: '53-56',
                  result: "You suffer the loss of a sense for several hours."
                },
                {
                  type: 'Between',
                  roll: '57-60',
                  result: "You lose your connection to magic for a day or so, and cannot perform rituals."
                },
                {
                  type: 'Between',
                  roll: '61-64',
                  result: "Your ritual affects the target in an unexpected and problematic way."
                },
                {
                  type: 'Between',
                  roll: '65-68',
                  result: "Your ritual reveals a surprising and troubling truth."
                },
                {
                  type: 'Between',
                  roll: '69-72',
                  result: "You are tempted by dark powers."
                },
                {
                  type: 'Between',
                  roll: '73-76',
                  result: "You see a troubling vision of your future."
                },
                {
                  type: 'Between',
                  roll: '77-80',
                  result: "You can't perform this ritual again until you acquire an important component."
                },
                {
                  type: 'Between',
                  roll: '81-84',
                  result: "You develop a strange fear or compulsion."
                },
                {
                  type: 'Between',
                  roll: '85-88',
                  result: "Your ritual causes creatures to exhibit strange or aggressive behavior."
                },
                {
                  type: 'Between',
                  roll: '89-92',
                  result: "You are tormented by an apparition from your past."
                },
                {
                  type: 'Between',
                  roll: '93-96',
                  result: "You are wracked with sudden sickness."
                },
                {
                  type: 'Between',
                  roll: '97-100',
                  result: "Roll twice more on this table. Both results occur. If they are the same result, make it worse."
                }
              ]
            },
            challenge_rank: {
              name: 'challenge_rank',
              roll_id: 'ChallengeRank',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-20',
                  result: "Troublesome"
                },
                {
                  type: 'Between',
                  roll: '21-55',
                  result: "Dangerous"
                },
                {
                  type: 'Between',
                  roll: '56-80',
                  result: "Formidable"
                },
                {
                  type: 'Between',
                  roll: '81-93',
                  result: "Extreme"
                },
                {
                  type: 'Between',
                  roll: '94-100',
                  result: "Epic"
                }
              ]
            }
          }
        },
        location: {
          oracles: {
            region: {
              name: 'region',
              roll_id: 'Region',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-12',
                  result: "Barrier Islands"
                },
                {
                  type: 'Between',
                  roll: '13-24',
                  result: "Ragged Coast"
                },
                {
                  type: 'Between',
                  roll: '25-34',
                  result: "Deep Wilds"
                },
                {
                  type: 'Between',
                  roll: '35-46',
                  result: "Flooded Lands"
                },
                {
                  type: 'Between',
                  roll: '47-60',
                  result: "Havens"
                },
                {
                  type: 'Between',
                  roll: '61-72',
                  result: "Hinterlands"
                },
                {
                  type: 'Between',
                  roll: '73-84',
                  result: "Tempest Hills"
                },
                {
                  type: 'Between',
                  roll: '85-94',
                  result: "Veiled Mountains"
                },
                {
                  type: 'Between',
                  roll: '95-99',
                  result: "Shattered Wastes"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Elsewhere"
                }
              ]
            },
            location: {
              name: 'location',
              roll_id: 'Location',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Total',
                  roll: '1',
                  result: "Hideout"
                },
                {
                  type: 'Total',
                  roll: '2',
                  result: "Ruin"
                },
                {
                  type: 'Total',
                  roll: '3',
                  result: "Mine"
                },
                {
                  type: 'Total',
                  roll: '4',
                  result: "Waste"
                },
                {
                  type: 'Total',
                  roll: '5',
                  result: "Mystical Site"
                },
                {
                  type: 'Total',
                  roll: '6',
                  result: "Path"
                },
                {
                  type: 'Total',
                  roll: '7',
                  result: "Outpost"
                },
                {
                  type: 'Total',
                  roll: '8',
                  result: "Wall"
                },
                {
                  type: 'Total',
                  roll: '9',
                  result: "Battlefield"
                },
                {
                  type: 'Total',
                  roll: '10',
                  result: "Hovel"
                },
                {
                  type: 'Total',
                  roll: '11',
                  result: "Spring"
                },
                {
                  type: 'Total',
                  roll: '12',
                  result: "Lair"
                },
                {
                  type: 'Total',
                  roll: '13',
                  result: "Fort"
                },
                {
                  type: 'Total',
                  roll: '14',
                  result: "Bridge"
                },
                {
                  type: 'Total',
                  roll: '15',
                  result: "Camp"
                },
                {
                  type: 'Total',
                  roll: '16',
                  result: "Cairn/Grave"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Caravan"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Waterfall"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Cave"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Swamp"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Fen"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Ravine"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Road"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Tree"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Pond"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Fields"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Marsh"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Steading"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Rapids"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Pass"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Trail"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Glade"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Plain"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Ridge"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Cliff"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Grove"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Village"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Moor"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Thicket"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "River Ford"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Valley"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Bay/Fjord"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Foothills"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Lake"
                },
                {
                  type: 'Between',
                  roll: '73-75',
                  result: "River"
                },
                {
                  type: 'Between',
                  roll: '76-79',
                  result: "Forest"
                },
                {
                  type: 'Between',
                  roll: '80-83',
                  result: "Coast"
                },
                {
                  type: 'Between',
                  roll: '84-88',
                  result: "Hill"
                },
                {
                  type: 'Between',
                  roll: '89-93',
                  result: "Mountain"
                },
                {
                  type: 'Between',
                  roll: '94-99',
                  result: "Woods"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Anomaly"
                }
              ]
            },
            coastal_waters_location: {
              name: 'coastal_waters_location',
              roll_id: 'CoastalWatersLocation',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Total',
                  roll: '1',
                  result: "Fleet"
                },
                {
                  type: 'Total',
                  roll: '2',
                  result: "Sargassum"
                },
                {
                  type: 'Total',
                  roll: '3',
                  result: "Flotsam"
                },
                {
                  type: 'Total',
                  roll: '4',
                  result: "Mystical Site"
                },
                {
                  type: 'Total',
                  roll: '5',
                  result: "Lair"
                },
                {
                  type: 'Between',
                  roll: '6-10',
                  result: "Wreck"
                },
                {
                  type: 'Between',
                  roll: '11-15',
                  result: "Harbor"
                },
                {
                  type: 'Between',
                  roll: '16-23',
                  result: "Ship"
                },
                {
                  type: 'Between',
                  roll: '24-30',
                  result: "Rocks"
                },
                {
                  type: 'Between',
                  roll: '31-38',
                  result: "Fjord"
                },
                {
                  type: 'Between',
                  roll: '39-46',
                  result: "Estuary"
                },
                {
                  type: 'Between',
                  roll: '47-54',
                  result: "Cove"
                },
                {
                  type: 'Between',
                  roll: '55-62',
                  result: "Bay"
                },
                {
                  type: 'Between',
                  roll: '63-70',
                  result: "Ice"
                },
                {
                  type: 'Between',
                  roll: '71-85',
                  result: "Island"
                },
                {
                  type: 'Between',
                  roll: '86-99',
                  result: "Open Water"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Anomaly"
                }
              ]
            },
            descriptor: {
              name: 'descriptor',
              roll_id: 'Descriptor',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "High"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Remote"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Exposed"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Small"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Broken"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Diverse"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Rough"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Dark"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Shadowy"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Contested"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Grim"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Wild"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Fertile"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Blocked"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Ancient"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Perilous"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Hidden"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Occupied"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Rich"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Big"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Savage"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Defended"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Withered"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Mystical"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Inaccessible"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Protected"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Abandoned"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Wide"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Foul"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Dead"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Ruined"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Barren"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Cold"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Blighted"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Low"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Beautiful"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Abundant"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Lush"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Flooded"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Empty"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Strange"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Corrupted"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Peaceful"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Forgotten"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Expansive"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Settled"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Dense"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Civilized"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Desolate"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Isolated"
                }
              ]
            },
            settlement_name: {
              name: 'settlement_name',
              roll_id: 'SettlementName',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-15',
                  result: "A feature of the landscape. Envision what it is. What makes it unusual or distinctive?"
                },
                {
                  type: 'Between',
                  roll: '16-30',
                  result: "A manmade edifice. What is it? Why is it important to this settlement’s history?"
                },
                {
                  type: 'Between',
                  roll: '31-45',
                  result: "A creature. Why have the people of this settlement chosen this creature as their totem? How is it represented in art or rituals?"
                },
                {
                  type: 'Between',
                  roll: '46-60',
                  result: "A historical event. What happened here? What place or practice commemorates this event?"
                },
                {
                  type: 'Between',
                  roll: '61-75',
                  result: "A word in an Old World language. What culture is represented by this word? What does it translate to?"
                },
                {
                  type: 'Between',
                  roll: '76-90',
                  result: "A season or environmental aspect. What influence does the weather have on this settlement?"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Something Else..."
                }
              ]
            },
            quick_settlement_name: {
              oracle_group: '2',
              name: 'quick_settlement_name',
              roll_id: 'QuickSettlementName',
              dice: '1',
              sub_oracles: [
                {
                  name: 'quick_settlement_name_prefix',
                  roll_id: 'QuickSettlementNamePrefix',
                  dice: 'd100',
                  columns: '1',
                  rows: [
                    {
                      type: 'Between',
                      roll: '1-4',
                      result: "Bleak"
                    },
                    {
                      type: 'Between',
                      roll: '5-8',
                      result: "Green"
                    },
                    {
                      type: 'Between',
                      roll: '9-12',
                      result: "Wolf"
                    },
                    {
                      type: 'Between',
                      roll: '13-16',
                      result: "Raven"
                    },
                    {
                      type: 'Between',
                      roll: '17-20',
                      result: "Gray"
                    },
                    {
                      type: 'Between',
                      roll: '21-24',
                      result: "Red"
                    },
                    {
                      type: 'Between',
                      roll: '25-28',
                      result: "Axe"
                    },
                    {
                      type: 'Between',
                      roll: '29-32',
                      result: "Great"
                    },
                    {
                      type: 'Between',
                      roll: '33-36',
                      result: "Wood"
                    },
                    {
                      type: 'Between',
                      roll: '37-40',
                      result: "Low"
                    },
                    {
                      type: 'Between',
                      roll: '41-44',
                      result: "White"
                    },
                    {
                      type: 'Between',
                      roll: '45-48',
                      result: "Storm"
                    },
                    {
                      type: 'Between',
                      roll: '49-52',
                      result: "Black"
                    },
                    {
                      type: 'Between',
                      roll: '53-56',
                      result: "Mourn"
                    },
                    {
                      type: 'Between',
                      roll: '57-60',
                      result: "New"
                    },
                    {
                      type: 'Between',
                      roll: '61-64',
                      result: "Stone"
                    },
                    {
                      type: 'Between',
                      roll: '65-68',
                      result: "Grim"
                    },
                    {
                      type: 'Between',
                      roll: '69-72',
                      result: "Lost"
                    },
                    {
                      type: 'Between',
                      roll: '73-76',
                      result: "High"
                    },
                    {
                      type: 'Between',
                      roll: '77-80',
                      result: "Rock"
                    },
                    {
                      type: 'Between',
                      roll: '81-84',
                      result: "Shield"
                    },
                    {
                      type: 'Between',
                      roll: '85-88',
                      result: "Sword"
                    },
                    {
                      type: 'Between',
                      roll: '89-92',
                      result: "Frost"
                    },
                    {
                      type: 'Between',
                      roll: '93-96',
                      result: "Thorn"
                    },
                    {
                      type: 'Between',
                      roll: '97-100',
                      result: "Long"
                    }
                  ]
                },
                {
                  name: 'quick_settlement_name_suffix',
                  roll_id: 'QuickSettlementNameSuffix',
                  dice: 'd100',
                  columns: '1',
                  rows: [
                    {
                      type: 'Between',
                      roll: '1-4',
                      result: "moor"
                    },
                    {
                      type: 'Between',
                      roll: '5-8',
                      result: "ford"
                    },
                    {
                      type: 'Between',
                      roll: '9-12',
                      result: "crag"
                    },
                    {
                      type: 'Between',
                      roll: '13-16',
                      result: "watch"
                    },
                    {
                      type: 'Between',
                      roll: '17-20',
                      result: "hope"
                    },
                    {
                      type: 'Between',
                      roll: '21-24',
                      result: "wood"
                    },
                    {
                      type: 'Between',
                      roll: '25-28',
                      result: "ridge"
                    },
                    {
                      type: 'Between',
                      roll: '29-32',
                      result: "stone"
                    },
                    {
                      type: 'Between',
                      roll: '33-36',
                      result: "haven"
                    },
                    {
                      type: 'Between',
                      roll: '37-40',
                      result: "fall(s)"
                    },
                    {
                      type: 'Between',
                      roll: '41-44',
                      result: "river"
                    },
                    {
                      type: 'Between',
                      roll: '45-48',
                      result: "field"
                    },
                    {
                      type: 'Between',
                      roll: '49-52',
                      result: "hill"
                    },
                    {
                      type: 'Between',
                      roll: '53-56',
                      result: "bridge"
                    },
                    {
                      type: 'Between',
                      roll: '57-60',
                      result: "mark"
                    },
                    {
                      type: 'Between',
                      roll: '61-64',
                      result: "cairn"
                    },
                    {
                      type: 'Between',
                      roll: '65-68',
                      result: "land"
                    },
                    {
                      type: 'Between',
                      roll: '69-72',
                      result: "hall"
                    },
                    {
                      type: 'Between',
                      roll: '73-76',
                      result: "mount"
                    },
                    {
                      type: 'Between',
                      roll: '77-80',
                      result: "rock"
                    },
                    {
                      type: 'Between',
                      roll: '81-84',
                      result: "brook"
                    },
                    {
                      type: 'Between',
                      roll: '85-88',
                      result: "barrow"
                    },
                    {
                      type: 'Between',
                      roll: '89-92',
                      result: "stead"
                    },
                    {
                      type: 'Between',
                      roll: '93-96',
                      result: "home"
                    },
                    {
                      type: 'Between',
                      roll: '97-100',
                      result: "wick"
                    }
                  ]
                }
              ]
            }
          }
        },
        character: {
          oracles: {
            role: {
              name: 'role',
              roll_id: 'Role',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Criminal"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Healer"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Bandit"
                },
                {
                  type: 'Between',
                  roll: '7-9',
                  result: "Guide"
                },
                {
                  type: 'Between',
                  roll: '10-12',
                  result: "Performer"
                },
                {
                  type: 'Between',
                  roll: '13-15',
                  result: "Miner"
                },
                {
                  type: 'Between',
                  roll: '16-18',
                  result: "Mercenary"
                },
                {
                  type: 'Between',
                  roll: '19-21',
                  result: "Outcast"
                },
                {
                  type: 'Between',
                  roll: '22-24',
                  result: "Vagrant"
                },
                {
                  type: 'Between',
                  roll: '25-27',
                  result: "Forester"
                },
                {
                  type: 'Between',
                  roll: '28-30',
                  result: "Traveler"
                },
                {
                  type: 'Between',
                  roll: '31-33',
                  result: "Mystic"
                },
                {
                  type: 'Between',
                  roll: '34-36',
                  result: "Priest"
                },
                {
                  type: 'Between',
                  roll: '37-39',
                  result: "Sailor"
                },
                {
                  type: 'Between',
                  roll: '40-42',
                  result: "Pilgrim"
                },
                {
                  type: 'Between',
                  roll: '43-45',
                  result: "Thief"
                },
                {
                  type: 'Between',
                  roll: '46-48',
                  result: "Adventurer"
                },
                {
                  type: 'Between',
                  roll: '49-51',
                  result: "Forager"
                },
                {
                  type: 'Between',
                  roll: '52-54',
                  result: "Leader"
                },
                {
                  type: 'Between',
                  roll: '55-58',
                  result: "Guard"
                },
                {
                  type: 'Between',
                  roll: '59-62',
                  result: "Artisan"
                },
                {
                  type: 'Between',
                  roll: '63-66',
                  result: "Scout"
                },
                {
                  type: 'Between',
                  roll: '67-70',
                  result: "Herder"
                },
                {
                  type: 'Between',
                  roll: '71-74',
                  result: "Fisher"
                },
                {
                  type: 'Between',
                  roll: '75-79',
                  result: "Warrior"
                },
                {
                  type: 'Between',
                  roll: '80-84',
                  result: "Hunter"
                },
                {
                  type: 'Between',
                  roll: '85-89',
                  result: "Raider"
                },
                {
                  type: 'Between',
                  roll: '90-94',
                  result: "Trader"
                },
                {
                  type: 'Between',
                  roll: '95-99',
                  result: "Farmer"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Unusual role"
                }
              ]
            },
            goal: {
              name: 'goal',
              roll_id: 'Goal',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-3',
                  result: "Obtain an object"
                },
                {
                  type: 'Between',
                  roll: '4-6',
                  result: "Make an agreement"
                },
                {
                  type: 'Between',
                  roll: '7-9',
                  result: "Build a relationship"
                },
                {
                  type: 'Between',
                  roll: '10-12',
                  result: "Undermine a relationship"
                },
                {
                  type: 'Between',
                  roll: '13-15',
                  result: "Seek a truth"
                },
                {
                  type: 'Between',
                  roll: '16-18',
                  result: "Pay a debt"
                },
                {
                  type: 'Between',
                  roll: '19-21',
                  result: "Refute a falsehood"
                },
                {
                  type: 'Between',
                  roll: '22-24',
                  result: "Harm a rival"
                },
                {
                  type: 'Between',
                  roll: '25-27',
                  result: "Cure an ill"
                },
                {
                  type: 'Between',
                  roll: '28-30',
                  result: "Find a person"
                },
                {
                  type: 'Between',
                  roll: '31-33',
                  result: "Find a home"
                },
                {
                  type: 'Between',
                  roll: '34-36',
                  result: "Seize power"
                },
                {
                  type: 'Between',
                  roll: '37-39',
                  result: "Restore a relationship"
                },
                {
                  type: 'Between',
                  roll: '40-42',
                  result: "Create an item"
                },
                {
                  type: 'Between',
                  roll: '43-45',
                  result: "Travel to a place"
                },
                {
                  type: 'Between',
                  roll: '46-48',
                  result: "Secure provisions"
                },
                {
                  type: 'Between',
                  roll: '49-51',
                  result: "Rebel against power"
                },
                {
                  type: 'Between',
                  roll: '52-54',
                  result: "Collect a debt"
                },
                {
                  type: 'Between',
                  roll: '55-57',
                  result: "Protect a secret"
                },
                {
                  type: 'Between',
                  roll: '58-60',
                  result: "Spread faith"
                },
                {
                  type: 'Between',
                  roll: '61-63',
                  result: "Enrich themselves"
                },
                {
                  type: 'Between',
                  roll: '64-66',
                  result: "Protect a person"
                },
                {
                  type: 'Between',
                  roll: '67-69',
                  result: "Protect the status quo"
                },
                {
                  type: 'Between',
                  roll: '70-72',
                  result: "Advance status"
                },
                {
                  type: 'Between',
                  roll: '73-75',
                  result: "Defend a place"
                },
                {
                  type: 'Between',
                  roll: '76-78',
                  result: "Avenge a wrong"
                },
                {
                  type: 'Between',
                  roll: '79-81',
                  result: "Fulfill a duty"
                },
                {
                  type: 'Between',
                  roll: '82-84',
                  result: "Gain knowledge"
                },
                {
                  type: 'Between',
                  roll: '85-87',
                  result: "Prove worthiness"
                },
                {
                  type: 'Between',
                  roll: '88-90',
                  result: "Find redemption"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Escape from something"
                },
                {
                  type: 'Between',
                  roll: '93-95',
                  result: "Resolve a dispute"
                },
                {
                  type: 'Between',
                  roll: '96-100',
                  result: "Roll twice"
                }
              ]
            },
            descriptor: {
              name: 'descriptor',
              roll_id: 'Descriptor',
              dice: 'd100',
              columns: '3',
              rows: [
                {
                  type: 'Total',
                  roll: '1',
                  result: "Stoic"
                },
                {
                  type: 'Total',
                  roll: '2',
                  result: "Attractive"
                },
                {
                  type: 'Total',
                  roll: '3',
                  result: "Passive"
                },
                {
                  type: 'Total',
                  roll: '4',
                  result: "Aloof"
                },
                {
                  type: 'Total',
                  roll: '5',
                  result: "Affectionate"
                },
                {
                  type: 'Total',
                  roll: '6',
                  result: "Generous"
                },
                {
                  type: 'Total',
                  roll: '7',
                  result: "Smug"
                },
                {
                  type: 'Total',
                  roll: '8',
                  result: "Armed"
                },
                {
                  type: 'Total',
                  roll: '9',
                  result: "Clever"
                },
                {
                  type: 'Total',
                  roll: '10',
                  result: "Brave"
                },
                {
                  type: 'Total',
                  roll: '11',
                  result: "Ugly"
                },
                {
                  type: 'Total',
                  roll: '12',
                  result: "Sociable"
                },
                {
                  type: 'Total',
                  roll: '13',
                  result: "Doomed"
                },
                {
                  type: 'Total',
                  roll: '14',
                  result: "Connected"
                },
                {
                  type: 'Total',
                  roll: '15',
                  result: "Bold"
                },
                {
                  type: 'Total',
                  roll: '16',
                  result: "Jealous"
                },
                {
                  type: 'Total',
                  roll: '17',
                  result: "Angry"
                },
                {
                  type: 'Total',
                  roll: '18',
                  result: "Active"
                },
                {
                  type: 'Total',
                  roll: '19',
                  result: "Suspicious"
                },
                {
                  type: 'Total',
                  roll: '20',
                  result: "Hostile"
                },
                {
                  type: 'Total',
                  roll: '21',
                  result: "Hardhearted"
                },
                {
                  type: 'Total',
                  roll: '22',
                  result: "Successful"
                },
                {
                  type: 'Total',
                  roll: '23',
                  result: "Talented"
                },
                {
                  type: 'Total',
                  roll: '24',
                  result: "Experienced"
                },
                {
                  type: 'Total',
                  roll: '25',
                  result: "Deceitful"
                },
                {
                  type: 'Total',
                  roll: '26',
                  result: "Ambitious"
                },
                {
                  type: 'Total',
                  roll: '27',
                  result: "Aggressive"
                },
                {
                  type: 'Total',
                  roll: '28',
                  result: "Conceited"
                },
                {
                  type: 'Total',
                  roll: '29',
                  result: "Proud"
                },
                {
                  type: 'Total',
                  roll: '30',
                  result: "Stern"
                },
                {
                  type: 'Total',
                  roll: '31',
                  result: "Dependent"
                },
                {
                  type: 'Total',
                  roll: '32',
                  result: "Wary"
                },
                {
                  type: 'Total',
                  roll: '33',
                  result: "Strong"
                },
                {
                  type: 'Total',
                  roll: '34',
                  result: "Insightful"
                },
                {
                  type: 'Total',
                  roll: '35',
                  result: "Dangerous"
                },
                {
                  type: 'Total',
                  roll: '36',
                  result: "Quirky"
                },
                {
                  type: 'Total',
                  roll: '37',
                  result: "Cheery"
                },
                {
                  type: 'Total',
                  roll: '38',
                  result: "Disfigured"
                },
                {
                  type: 'Total',
                  roll: '39',
                  result: "Intolerant"
                },
                {
                  type: 'Total',
                  roll: '40',
                  result: "Skilled"
                },
                {
                  type: 'Total',
                  roll: '41',
                  result: "Stingy"
                },
                {
                  type: 'Total',
                  roll: '42',
                  result: "Timid"
                },
                {
                  type: 'Total',
                  roll: '43',
                  result: "Insensitive"
                },
                {
                  type: 'Total',
                  roll: '44',
                  result: "Wild"
                },
                {
                  type: 'Total',
                  roll: '45',
                  result: "Bitter"
                },
                {
                  type: 'Total',
                  roll: '46',
                  result: "Cunning"
                },
                {
                  type: 'Total',
                  roll: '47',
                  result: "Remorseful"
                },
                {
                  type: 'Total',
                  roll: '48',
                  result: "Kind"
                },
                {
                  type: 'Total',
                  roll: '49',
                  result: "Charming"
                },
                {
                  type: 'Total',
                  roll: '50',
                  result: "Oblivious"
                },
                {
                  type: 'Total',
                  roll: '51',
                  result: "Critical"
                },
                {
                  type: 'Total',
                  roll: '52',
                  result: "Cautious"
                },
                {
                  type: 'Total',
                  roll: '53',
                  result: "Resourceful"
                },
                {
                  type: 'Total',
                  roll: '54',
                  result: "Weary"
                },
                {
                  type: 'Total',
                  roll: '55',
                  result: "Wounded"
                },
                {
                  type: 'Total',
                  roll: '56',
                  result: "Anxious"
                },
                {
                  type: 'Total',
                  roll: '57',
                  result: "Powerful"
                },
                {
                  type: 'Total',
                  roll: '58',
                  result: "Athletic"
                },
                {
                  type: 'Total',
                  roll: '59',
                  result: "Driven"
                },
                {
                  type: 'Total',
                  roll: '60',
                  result: "Cruel"
                },
                {
                  type: 'Total',
                  roll: '61',
                  result: "Quiet"
                },
                {
                  type: 'Total',
                  roll: '62',
                  result: "Honest"
                },
                {
                  type: 'Total',
                  roll: '63',
                  result: "Infamous"
                },
                {
                  type: 'Total',
                  roll: '64',
                  result: "Dying"
                },
                {
                  type: 'Total',
                  roll: '65',
                  result: "Reclusive"
                },
                {
                  type: 'Total',
                  roll: '66',
                  result: "Artistic"
                },
                {
                  type: 'Total',
                  roll: '67',
                  result: "Disabled"
                },
                {
                  type: 'Total',
                  roll: '68',
                  result: "Confused"
                },
                {
                  type: 'Total',
                  roll: '69',
                  result: "Manipulative"
                },
                {
                  type: 'Total',
                  roll: '70',
                  result: "Relaxed"
                },
                {
                  type: 'Total',
                  roll: '71',
                  result: "Stealthy"
                },
                {
                  type: 'Total',
                  roll: '72',
                  result: "Confident"
                },
                {
                  type: 'Total',
                  roll: '73',
                  result: "Weak"
                },
                {
                  type: 'Total',
                  roll: '74',
                  result: "Friendly"
                },
                {
                  type: 'Total',
                  roll: '75',
                  result: "Wise"
                },
                {
                  type: 'Total',
                  roll: '76',
                  result: "Influential"
                },
                {
                  type: 'Total',
                  roll: '77',
                  result: "Young"
                },
                {
                  type: 'Total',
                  roll: '78',
                  result: "Adventurous"
                },
                {
                  type: 'Total',
                  roll: '79',
                  result: "Oppressed"
                },
                {
                  type: 'Total',
                  roll: '80',
                  result: "Vengeful"
                },
                {
                  type: 'Total',
                  roll: '81',
                  result: "Cooperative"
                },
                {
                  type: 'Total',
                  roll: '82',
                  result: "Armored"
                },
                {
                  type: 'Total',
                  roll: '83',
                  result: "Apathetic"
                },
                {
                  type: 'Total',
                  roll: '84',
                  result: "Determined"
                },
                {
                  type: 'Total',
                  roll: '85',
                  result: "Loyal"
                },
                {
                  type: 'Total',
                  roll: '86',
                  result: "Sick"
                },
                {
                  type: 'Total',
                  roll: '87',
                  result: "Religious"
                },
                {
                  type: 'Total',
                  roll: '88',
                  result: "Selfish"
                },
                {
                  type: 'Total',
                  roll: '89',
                  result: "Old"
                },
                {
                  type: 'Total',
                  roll: '90',
                  result: "Fervent"
                },
                {
                  type: 'Total',
                  roll: '91',
                  result: "Violent"
                },
                {
                  type: 'Total',
                  roll: '92',
                  result: "Agreeable"
                },
                {
                  type: 'Total',
                  roll: '93',
                  result: "Hot-tempered"
                },
                {
                  type: 'Total',
                  roll: '94',
                  result: "Stubborn"
                },
                {
                  type: 'Total',
                  roll: '95',
                  result: "Incompetent"
                },
                {
                  type: 'Total',
                  roll: '96',
                  result: "Greedy"
                },
                {
                  type: 'Total',
                  roll: '97',
                  result: "Cowardly"
                },
                {
                  type: 'Total',
                  roll: '98',
                  result: "Obsessed"
                },
                {
                  type: 'Total',
                  roll: '99',
                  result: "Careless"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Ironsworn"
                }
              ]
            },
            disposition: {
              name: 'disposition',
              roll_id: 'Disposition',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-6',
                  result: "Helpful"
                },
                {
                  type: 'Between',
                  roll: '7-13',
                  result: "Friendly"
                },
                {
                  type: 'Between',
                  roll: '14-20',
                  result: "Cooperative"
                },
                {
                  type: 'Between',
                  roll: '21-28',
                  result: "Curious"
                },
                {
                  type: 'Between',
                  roll: '29-36',
                  result: "Indifferent"
                },
                {
                  type: 'Between',
                  roll: '37-47',
                  result: "Suspicious"
                },
                {
                  type: 'Between',
                  roll: '48-57',
                  result: "Wanting"
                },
                {
                  type: 'Between',
                  roll: '58-67',
                  result: "Desperate"
                },
                {
                  type: 'Between',
                  roll: '68-76',
                  result: "Demanding"
                },
                {
                  type: 'Between',
                  roll: '77-85',
                  result: "Unfriendly"
                },
                {
                  type: 'Between',
                  roll: '86-93',
                  result: "Threatening"
                },
                {
                  type: 'Between',
                  roll: '94-100',
                  result: "Hostile"
                }
              ]
            },
            activity: {
              name: 'activity',
              roll_id: 'Activity',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Guarding"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Preserving"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Constructing"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Mending"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Assisting"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Securing"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Learning"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Sneaking"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Fleeing"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Sacrificing"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Creating"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Luring"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Hunting"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Seizing"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Bargaining"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Mimicking"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Tricking"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Tracking"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Escorting"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Hiding"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Raiding"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Socializing"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Exploring"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Journeying"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Supporting"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Avoiding"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Disabling"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Leading"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Assaulting"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Ensnaring"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Defending"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Recovering"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Patrolling"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Resting"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Distracting"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Leaving"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Fighting"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Ambushing"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Controlling"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Observing"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Gathering"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Suffering"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Threatening"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Searching"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Destroying"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Restoring"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Consuming"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Removing"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Inspecting"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Summoning"
                }
              ]
            }
          }
        },
        names: {
          oracles: {
            ironlander: {
              name: 'ironlander',
              roll_id: 'Ironlander',
              dice: 'd200',
              columns: '4',
              rows: [
                {
                  type: 'Total',
                  roll: '1',
                  result: "Solana"
                },
                {
                  type: 'Total',
                  roll: '2',
                  result: "Keelan"
                },
                {
                  type: 'Total',
                  roll: '3',
                  result: "Cadigan"
                },
                {
                  type: 'Total',
                  roll: '4',
                  result: "Sola"
                },
                {
                  type: 'Total',
                  roll: '5',
                  result: "Kodroth"
                },
                {
                  type: 'Total',
                  roll: '6',
                  result: "Kione"
                },
                {
                  type: 'Total',
                  roll: '7',
                  result: "Katja"
                },
                {
                  type: 'Total',
                  roll: '8',
                  result: "Tio"
                },
                {
                  type: 'Total',
                  roll: '9',
                  result: "Artiga"
                },
                {
                  type: 'Total',
                  roll: '10',
                  result: "Eos"
                },
                {
                  type: 'Total',
                  roll: '11',
                  result: "Bastien"
                },
                {
                  type: 'Total',
                  roll: '12',
                  result: "Elli"
                },
                {
                  type: 'Total',
                  roll: '13',
                  result: "Maura"
                },
                {
                  type: 'Total',
                  roll: '14',
                  result: "Haleema"
                },
                {
                  type: 'Total',
                  roll: '15',
                  result: "Abella"
                },
                {
                  type: 'Total',
                  roll: '16',
                  result: "Morter"
                },
                {
                  type: 'Total',
                  roll: '17',
                  result: "Wulan"
                },
                {
                  type: 'Total',
                  roll: '18',
                  result: "Mai"
                },
                {
                  type: 'Total',
                  roll: '19',
                  result: "Farina"
                },
                {
                  type: 'Total',
                  roll: '20',
                  result: "Pearce"
                },
                {
                  type: 'Total',
                  roll: '21',
                  result: "Wynne"
                },
                {
                  type: 'Total',
                  roll: '22',
                  result: "Haf"
                },
                {
                  type: 'Total',
                  roll: '23',
                  result: "Aeddon"
                },
                {
                  type: 'Total',
                  roll: '24',
                  result: "Khinara"
                },
                {
                  type: 'Total',
                  roll: '25',
                  result: "Milla"
                },
                {
                  type: 'Total',
                  roll: '26',
                  result: "Nakata"
                },
                {
                  type: 'Total',
                  roll: '27',
                  result: "Kynan"
                },
                {
                  type: 'Total',
                  roll: '28',
                  result: "Kiah"
                },
                {
                  type: 'Total',
                  roll: '29',
                  result: "Jaggar"
                },
                {
                  type: 'Total',
                  roll: '30',
                  result: "Beca"
                },
                {
                  type: 'Total',
                  roll: '31',
                  result: "Ikram"
                },
                {
                  type: 'Total',
                  roll: '32',
                  result: "Melia"
                },
                {
                  type: 'Total',
                  roll: '33',
                  result: "Sidan"
                },
                {
                  type: 'Total',
                  roll: '34',
                  result: "Deshi"
                },
                {
                  type: 'Total',
                  roll: '35',
                  result: "Tessa"
                },
                {
                  type: 'Total',
                  roll: '36',
                  result: "Sibila"
                },
                {
                  type: 'Total',
                  roll: '37',
                  result: "Morien"
                },
                {
                  type: 'Total',
                  roll: '38',
                  result: "Mona"
                },
                {
                  type: 'Total',
                  roll: '39',
                  result: "Padma"
                },
                {
                  type: 'Total',
                  roll: '40',
                  result: "Avella"
                },
                {
                  type: 'Total',
                  roll: '41',
                  result: "Naila"
                },
                {
                  type: 'Total',
                  roll: '42',
                  result: "Lio"
                },
                {
                  type: 'Total',
                  roll: '43',
                  result: "Cera"
                },
                {
                  type: 'Total',
                  roll: '44',
                  result: "Ithela"
                },
                {
                  type: 'Total',
                  roll: '45',
                  result: "Zhan"
                },
                {
                  type: 'Total',
                  roll: '46',
                  result: "Kaivan"
                },
                {
                  type: 'Total',
                  roll: '47',
                  result: "Valeri"
                },
                {
                  type: 'Total',
                  roll: '48',
                  result: "Hirsham"
                },
                {
                  type: 'Total',
                  roll: '49',
                  result: "Pemba"
                },
                {
                  type: 'Total',
                  roll: '50',
                  result: "Edda"
                },
                {
                  type: 'Total',
                  roll: '51',
                  result: "Lestara"
                },
                {
                  type: 'Total',
                  roll: '52',
                  result: "Lago"
                },
                {
                  type: 'Total',
                  roll: '53',
                  result: "Elstan"
                },
                {
                  type: 'Total',
                  roll: '54',
                  result: "Saskia"
                },
                {
                  type: 'Total',
                  roll: '55',
                  result: "Kabeera"
                },
                {
                  type: 'Total',
                  roll: '56',
                  result: "Caldas"
                },
                {
                  type: 'Total',
                  roll: '57',
                  result: "Nisus"
                },
                {
                  type: 'Total',
                  roll: '58',
                  result: "Serene"
                },
                {
                  type: 'Total',
                  roll: '59',
                  result: "Chenda"
                },
                {
                  type: 'Total',
                  roll: '60',
                  result: "Themon"
                },
                {
                  type: 'Total',
                  roll: '61',
                  result: "Erin"
                },
                {
                  type: 'Total',
                  roll: '62',
                  result: "Alban"
                },
                {
                  type: 'Total',
                  roll: '63',
                  result: "Parcell"
                },
                {
                  type: 'Total',
                  roll: '64',
                  result: "Jelma"
                },
                {
                  type: 'Total',
                  roll: '65',
                  result: "Willa"
                },
                {
                  type: 'Total',
                  roll: '66',
                  result: "Nadira"
                },
                {
                  type: 'Total',
                  roll: '67',
                  result: "Gwen"
                },
                {
                  type: 'Total',
                  roll: '68',
                  result: "Amara"
                },
                {
                  type: 'Total',
                  roll: '69',
                  result: "Masias"
                },
                {
                  type: 'Total',
                  roll: '70',
                  result: "Kanno"
                },
                {
                  type: 'Total',
                  roll: '71',
                  result: "Razeena"
                },
                {
                  type: 'Total',
                  roll: '72',
                  result: "Mira"
                },
                {
                  type: 'Total',
                  roll: '73',
                  result: "Perella"
                },
                {
                  type: 'Total',
                  roll: '74',
                  result: "Myrick"
                },
                {
                  type: 'Total',
                  roll: '75',
                  result: "Qamar"
                },
                {
                  type: 'Total',
                  roll: '76',
                  result: "Kormak"
                },
                {
                  type: 'Total',
                  roll: '77',
                  result: "Zura"
                },
                {
                  type: 'Total',
                  roll: '78',
                  result: "Zanita"
                },
                {
                  type: 'Total',
                  roll: '79',
                  result: "Brynn"
                },
                {
                  type: 'Total',
                  roll: '80',
                  result: "Tegan"
                },
                {
                  type: 'Total',
                  roll: '81',
                  result: "Pendry"
                },
                {
                  type: 'Total',
                  roll: '82',
                  result: "Quinn"
                },
                {
                  type: 'Total',
                  roll: '83',
                  result: "Fanir"
                },
                {
                  type: 'Total',
                  roll: '84',
                  result: "Glain"
                },
                {
                  type: 'Total',
                  roll: '85',
                  result: "Emelyn"
                },
                {
                  type: 'Total',
                  roll: '86',
                  result: "Kendi"
                },
                {
                  type: 'Total',
                  roll: '87',
                  result: "Althus"
                },
                {
                  type: 'Total',
                  roll: '88',
                  result: "Leela"
                },
                {
                  type: 'Total',
                  roll: '89',
                  result: "Ishana"
                },
                {
                  type: 'Total',
                  roll: '90',
                  result: "Flint"
                },
                {
                  type: 'Total',
                  roll: '91',
                  result: "Delkash"
                },
                {
                  type: 'Total',
                  roll: '92',
                  result: "Nia"
                },
                {
                  type: 'Total',
                  roll: '93',
                  result: "Nan"
                },
                {
                  type: 'Total',
                  roll: '94',
                  result: "Keeara"
                },
                {
                  type: 'Total',
                  roll: '95',
                  result: "Katania"
                },
                {
                  type: 'Total',
                  roll: '96',
                  result: "Morell"
                },
                {
                  type: 'Total',
                  roll: '97',
                  result: "Temir"
                },
                {
                  type: 'Total',
                  roll: '98',
                  result: "Bas"
                },
                {
                  type: 'Total',
                  roll: '99',
                  result: "Sabine"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Tallus"
                },
                {
                  type: 'Total',
                  roll: '101',
                  result: "Segura"
                },
                {
                  type: 'Total',
                  roll: '102',
                  result: "Gethin"
                },
                {
                  type: 'Total',
                  roll: '103',
                  result: "Bataar"
                },
                {
                  type: 'Total',
                  roll: '104',
                  result: "Basira"
                },
                {
                  type: 'Total',
                  roll: '105',
                  result: "Joa"
                },
                {
                  type: 'Total',
                  roll: '106',
                  result: "Glynn"
                },
                {
                  type: 'Total',
                  roll: '107',
                  result: "Toran"
                },
                {
                  type: 'Total',
                  roll: '108',
                  result: "Arasen"
                },
                {
                  type: 'Total',
                  roll: '109',
                  result: "Kuron"
                },
                {
                  type: 'Total',
                  roll: '110',
                  result: "Griff"
                },
                {
                  type: 'Total',
                  roll: '111',
                  result: "Owena"
                },
                {
                  type: 'Total',
                  roll: '112',
                  result: "Adda"
                },
                {
                  type: 'Total',
                  roll: '113',
                  result: "Euros"
                },
                {
                  type: 'Total',
                  roll: '114',
                  result: "Kova"
                },
                {
                  type: 'Total',
                  roll: '115',
                  result: "Kara"
                },
                {
                  type: 'Total',
                  roll: '116',
                  result: "Morgan"
                },
                {
                  type: 'Total',
                  roll: '117',
                  result: "Nanda"
                },
                {
                  type: 'Total',
                  roll: '118',
                  result: "Tamara"
                },
                {
                  type: 'Total',
                  roll: '119',
                  result: "Asha"
                },
                {
                  type: 'Total',
                  roll: '120',
                  result: "Delos"
                },
                {
                  type: 'Total',
                  roll: '121',
                  result: "Torgan"
                },
                {
                  type: 'Total',
                  roll: '122',
                  result: "Makari"
                },
                {
                  type: 'Total',
                  roll: '123',
                  result: "Selva"
                },
                {
                  type: 'Total',
                  roll: '124',
                  result: "Kimura"
                },
                {
                  type: 'Total',
                  roll: '125',
                  result: "Rhian"
                },
                {
                  type: 'Total',
                  roll: '126',
                  result: "Tristan"
                },
                {
                  type: 'Total',
                  roll: '127',
                  result: "Siorra"
                },
                {
                  type: 'Total',
                  roll: '128',
                  result: "Sayer"
                },
                {
                  type: 'Total',
                  roll: '129',
                  result: "Cortina"
                },
                {
                  type: 'Total',
                  roll: '130',
                  result: "Vesna"
                },
                {
                  type: 'Total',
                  roll: '131',
                  result: "Kataka"
                },
                {
                  type: 'Total',
                  roll: '132',
                  result: "Keyshia"
                },
                {
                  type: 'Total',
                  roll: '133',
                  result: "Mila"
                },
                {
                  type: 'Total',
                  roll: '134',
                  result: "Lili"
                },
                {
                  type: 'Total',
                  roll: '135',
                  result: "Vigo"
                },
                {
                  type: 'Total',
                  roll: '136',
                  result: "Sadia"
                },
                {
                  type: 'Total',
                  roll: '137',
                  result: "Malik"
                },
                {
                  type: 'Total',
                  roll: '138',
                  result: "Dag"
                },
                {
                  type: 'Total',
                  roll: '139',
                  result: "Kuno"
                },
                {
                  type: 'Total',
                  roll: '140',
                  result: "Reva"
                },
                {
                  type: 'Total',
                  roll: '141',
                  result: "Kai"
                },
                {
                  type: 'Total',
                  roll: '142',
                  result: "Kalina"
                },
                {
                  type: 'Total',
                  roll: '143',
                  result: "Jihan"
                },
                {
                  type: 'Total',
                  roll: '144',
                  result: "Hennion"
                },
                {
                  type: 'Total',
                  roll: '145',
                  result: "Abram"
                },
                {
                  type: 'Total',
                  roll: '146',
                  result: "Aida"
                },
                {
                  type: 'Total',
                  roll: '147',
                  result: "Myrtle"
                },
                {
                  type: 'Total',
                  roll: '148',
                  result: "Nekun"
                },
                {
                  type: 'Total',
                  roll: '149',
                  result: "Menna"
                },
                {
                  type: 'Total',
                  roll: '150',
                  result: "Tahir"
                },
                {
                  type: 'Total',
                  roll: '151',
                  result: "Sarria"
                },
                {
                  type: 'Total',
                  roll: '152',
                  result: "Nakura"
                },
                {
                  type: 'Total',
                  roll: '153',
                  result: "Akiya"
                },
                {
                  type: 'Total',
                  roll: '154',
                  result: "Talan"
                },
                {
                  type: 'Total',
                  roll: '155',
                  result: "Mattick"
                },
                {
                  type: 'Total',
                  roll: '156',
                  result: "Okoth"
                },
                {
                  type: 'Total',
                  roll: '157',
                  result: "Khulan"
                },
                {
                  type: 'Total',
                  roll: '158',
                  result: "Verena"
                },
                {
                  type: 'Total',
                  roll: '159',
                  result: "Beltran"
                },
                {
                  type: 'Total',
                  roll: '160',
                  result: "Del"
                },
                {
                  type: 'Total',
                  roll: '161',
                  result: "Ranna"
                },
                {
                  type: 'Total',
                  roll: '162',
                  result: "Alina"
                },
                {
                  type: 'Total',
                  roll: '163',
                  result: "Muna"
                },
                {
                  type: 'Total',
                  roll: '164',
                  result: "Mura"
                },
                {
                  type: 'Total',
                  roll: '165',
                  result: "Torrens"
                },
                {
                  type: 'Total',
                  roll: '166',
                  result: "Yuda"
                },
                {
                  type: 'Total',
                  roll: '167',
                  result: "Nazmi"
                },
                {
                  type: 'Total',
                  roll: '168',
                  result: "Ghalen"
                },
                {
                  type: 'Total',
                  roll: '169',
                  result: "Sarda"
                },
                {
                  type: 'Total',
                  roll: '170',
                  result: "Shona"
                },
                {
                  type: 'Total',
                  roll: '171',
                  result: "Kalidas"
                },
                {
                  type: 'Total',
                  roll: '172',
                  result: "Wena"
                },
                {
                  type: 'Total',
                  roll: '173',
                  result: "Sendra"
                },
                {
                  type: 'Total',
                  roll: '174',
                  result: "Kori"
                },
                {
                  type: 'Total',
                  roll: '175',
                  result: "Setara"
                },
                {
                  type: 'Total',
                  roll: '176',
                  result: "Lucia"
                },
                {
                  type: 'Total',
                  roll: '177',
                  result: "Maya"
                },
                {
                  type: 'Total',
                  roll: '178',
                  result: "Reema"
                },
                {
                  type: 'Total',
                  roll: '179',
                  result: "Yorath"
                },
                {
                  type: 'Total',
                  roll: '180',
                  result: "Rhoddri"
                },
                {
                  type: 'Total',
                  roll: '181',
                  result: "Shekhar"
                },
                {
                  type: 'Total',
                  roll: '182',
                  result: "Servan"
                },
                {
                  type: 'Total',
                  roll: '183',
                  result: "Reese"
                },
                {
                  type: 'Total',
                  roll: '184',
                  result: "Kenrick"
                },
                {
                  type: 'Total',
                  roll: '185',
                  result: "Indirra"
                },
                {
                  type: 'Total',
                  roll: '186',
                  result: "Giliana"
                },
                {
                  type: 'Total',
                  roll: '187',
                  result: "Jebran"
                },
                {
                  type: 'Total',
                  roll: '188',
                  result: "Kotama"
                },
                {
                  type: 'Total',
                  roll: '189',
                  result: "Fara"
                },
                {
                  type: 'Total',
                  roll: '190',
                  result: "Katrin"
                },
                {
                  type: 'Total',
                  roll: '191',
                  result: "Namba"
                },
                {
                  type: 'Total',
                  roll: '192',
                  result: "Lona"
                },
                {
                  type: 'Total',
                  roll: '193',
                  result: "Taylah"
                },
                {
                  type: 'Total',
                  roll: '194',
                  result: "Kato"
                },
                {
                  type: 'Total',
                  roll: '195',
                  result: "Esra"
                },
                {
                  type: 'Total',
                  roll: '196',
                  result: "Eleri"
                },
                {
                  type: 'Total',
                  roll: '197',
                  result: "Irsia"
                },
                {
                  type: 'Total',
                  roll: '198',
                  result: "Kayu"
                },
                {
                  type: 'Total',
                  roll: '199',
                  result: "Bevan"
                },
                {
                  type: 'Total',
                  roll: '200',
                  result: "Chandra"
                }
              ]
            },
            elf: {
              name: 'elf',
              roll_id: 'Elf',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Arsula"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Naidita"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Belesunna"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Vidarna"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Ninsunu"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Balathu"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Dorosi"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Gezera"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Zursan"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Seleeku"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Utamara"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Nebakay"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Dismashk"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Mitunu"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Atani"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Kinzura"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Sumula"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Ukames"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Ahmeshki"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Ilsit"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Mayatanay"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Etana"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Gamanna"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Nessana"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Uralar"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Tishetu"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Leucia"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Sutahe"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Dotani"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Uktannu"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Retenay"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Kendalanu"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Tahuta"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Mattissa"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Anatu"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Aralu"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Arakhi"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Ibrahem"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Sinosu"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Jemshida"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Visapni"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Hullata"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Sidura"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Kerihu"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Ereshki"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Cybela"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Anunna"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Otani"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Ditani"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Faraza"
                }
              ]
            },
            giant: {
              name: 'giant',
              roll_id: 'Giant',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-4',
                  result: "Chony"
                },
                {
                  type: 'Between',
                  roll: '5-8',
                  result: "Banda"
                },
                {
                  type: 'Between',
                  roll: '9-12',
                  result: "Jochu"
                },
                {
                  type: 'Between',
                  roll: '13-16',
                  result: "Kira"
                },
                {
                  type: 'Between',
                  roll: '17-20',
                  result: "Khatir"
                },
                {
                  type: 'Between',
                  roll: '21-24',
                  result: "Chaidu"
                },
                {
                  type: 'Between',
                  roll: '25-28',
                  result: "Atan"
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Buandu"
                },
                {
                  type: 'Between',
                  roll: '33-36',
                  result: "Javyn"
                },
                {
                  type: 'Between',
                  roll: '37-40',
                  result: "Khashin"
                },
                {
                  type: 'Between',
                  roll: '41-44',
                  result: "Bayara"
                },
                {
                  type: 'Between',
                  roll: '45-48',
                  result: "Temura"
                },
                {
                  type: 'Between',
                  roll: '49-52',
                  result: "Kidha"
                },
                {
                  type: 'Between',
                  roll: '53-56',
                  result: "Kathos"
                },
                {
                  type: 'Between',
                  roll: '57-60',
                  result: "Tanua"
                },
                {
                  type: 'Between',
                  roll: '61-64',
                  result: "Bashtu"
                },
                {
                  type: 'Between',
                  roll: '65-68',
                  result: "Jaran"
                },
                {
                  type: 'Between',
                  roll: '69-72',
                  result: "Othos"
                },
                {
                  type: 'Between',
                  roll: '73-76',
                  result: "Khutan"
                },
                {
                  type: 'Between',
                  roll: '77-80',
                  result: "Otaan"
                },
                {
                  type: 'Between',
                  roll: '81-84',
                  result: "Martu"
                },
                {
                  type: 'Between',
                  roll: '85-88',
                  result: "Baku"
                },
                {
                  type: 'Between',
                  roll: '89-92',
                  result: "Tuban"
                },
                {
                  type: 'Between',
                  roll: '93-96',
                  result: "Qudan"
                },
                {
                  type: 'Between',
                  roll: '97-100',
                  result: "Denua"
                }
              ]
            },
            varou: {
              name: 'varou',
              roll_id: 'Varou',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-4',
                  result: "Vata"
                },
                {
                  type: 'Between',
                  roll: '5-8',
                  result: "Zora"
                },
                {
                  type: 'Between',
                  roll: '9-12',
                  result: "Jasna"
                },
                {
                  type: 'Between',
                  roll: '13-16',
                  result: "Charna"
                },
                {
                  type: 'Between',
                  roll: '17-20',
                  result: "Tana"
                },
                {
                  type: 'Between',
                  roll: '21-24',
                  result: "Soveen"
                },
                {
                  type: 'Between',
                  roll: '25-28',
                  result: "Radka"
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Zlata"
                },
                {
                  type: 'Between',
                  roll: '33-36',
                  result: "Leesla"
                },
                {
                  type: 'Between',
                  roll: '37-40',
                  result: "Byna"
                },
                {
                  type: 'Between',
                  roll: '41-44',
                  result: "Meeka"
                },
                {
                  type: 'Between',
                  roll: '45-48',
                  result: "Iskra"
                },
                {
                  type: 'Between',
                  roll: '49-52',
                  result: "Jarek"
                },
                {
                  type: 'Between',
                  roll: '53-56',
                  result: "Darva"
                },
                {
                  type: 'Between',
                  roll: '57-60',
                  result: "Neda"
                },
                {
                  type: 'Between',
                  roll: '61-64',
                  result: "Keha"
                },
                {
                  type: 'Between',
                  roll: '65-68',
                  result: "Zhivka"
                },
                {
                  type: 'Between',
                  roll: '69-72',
                  result: "Kvata"
                },
                {
                  type: 'Between',
                  roll: '73-76',
                  result: "Staysa"
                },
                {
                  type: 'Between',
                  roll: '77-80',
                  result: "Evka"
                },
                {
                  type: 'Between',
                  roll: '81-84',
                  result: "Vuksha"
                },
                {
                  type: 'Between',
                  roll: '85-88',
                  result: "Muko"
                },
                {
                  type: 'Between',
                  roll: '89-92',
                  result: "Dreko"
                },
                {
                  type: 'Between',
                  roll: '93-96',
                  result: "Aleko"
                },
                {
                  type: 'Between',
                  roll: '97-100',
                  result: "Vojan"
                }
              ]
            },
            troll: {
              name: 'troll',
              roll_id: 'Troll',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-4',
                  result: "Rattle"
                },
                {
                  type: 'Between',
                  roll: '5-8',
                  result: "Scratch"
                },
                {
                  type: 'Between',
                  roll: '9-12',
                  result: "Wallow"
                },
                {
                  type: 'Between',
                  roll: '13-16',
                  result: "Groak"
                },
                {
                  type: 'Between',
                  roll: '17-20',
                  result: "Gimble"
                },
                {
                  type: 'Between',
                  roll: '21-24',
                  result: "Scar"
                },
                {
                  type: 'Between',
                  roll: '25-28',
                  result: "Cratch"
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Creech"
                },
                {
                  type: 'Between',
                  roll: '33-36',
                  result: "Shush"
                },
                {
                  type: 'Between',
                  roll: '37-40',
                  result: "Glush"
                },
                {
                  type: 'Between',
                  roll: '41-44',
                  result: "Slar"
                },
                {
                  type: 'Between',
                  roll: '45-48',
                  result: "Gnash"
                },
                {
                  type: 'Between',
                  roll: '49-52',
                  result: "Stoad"
                },
                {
                  type: 'Between',
                  roll: '53-56',
                  result: "Grig"
                },
                {
                  type: 'Between',
                  roll: '57-60',
                  result: "Bleat"
                },
                {
                  type: 'Between',
                  roll: '61-64',
                  result: "Chortle"
                },
                {
                  type: 'Between',
                  roll: '65-68',
                  result: "Cluck"
                },
                {
                  type: 'Between',
                  roll: '69-72',
                  result: "Slith"
                },
                {
                  type: 'Between',
                  roll: '73-76',
                  result: "Mongo"
                },
                {
                  type: 'Between',
                  roll: '77-80',
                  result: "Creak"
                },
                {
                  type: 'Between',
                  roll: '81-84',
                  result: "Burble"
                },
                {
                  type: 'Between',
                  roll: '85-88',
                  result: "Vrusk"
                },
                {
                  type: 'Between',
                  roll: '89-92',
                  result: "Snuffle"
                },
                {
                  type: 'Between',
                  roll: '93-96',
                  result: "Leech"
                },
                {
                  type: 'Between',
                  roll: '97-100',
                  result: "Herk"
                }
              ]
            }
          }
        },
        moves: {
          oracles: {
            ask_the_oracle: {
              name: 'ask_the_oracle',
              roll_id: 'AskTheOracle',
              oracle_move: true
            },
            pay_the_price: {
              name: 'pay_the_price',
              roll_id: 'PayThePrice',
              oracle_move: true
            }
          }
        }
      },
      delve: {
        feature: {
          oracles: {
            aspect: {
              name: 'aspect',
              roll_id: 'Aspect',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Blocked"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Crafted"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Ancient"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Sunken"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Trapped"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Secret"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Toxic"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Ruined"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Defended"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Decaying"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Marked"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Guarded"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Inaccessible"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Foreboding"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Veiled"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Deep"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Depleted"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Foul"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Elevated"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Moving"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Unnatural"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Active"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Confined"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Fortified"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Collapsed"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Isolated"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Destroyed"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Open"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Sacred"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Flooded"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Complex"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Abundant"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Hidden"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Expansive"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Mysterious"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Unstable"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Fragile"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Broken"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Ensnaring"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Pillaged"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Sealed"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Makeshift"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Treacherous"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Natural"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Dead"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Unusual"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Abandoned"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Deadly"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Forgotten"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Mystical"
                }
              ]
            },
            focus: {
              name: 'focus',
              roll_id: 'Focus',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Attack"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Threshold"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Boundary"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Alarm"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Exit"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Passage"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Crossing"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Trigger"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Trap"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Hideaway"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Nature"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Sign"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Refuge"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Valuables"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Breach"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Route"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Location"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Trail"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Supply"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "History"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Prisoner"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Habitation"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Debris"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Creature"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Lair"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Person"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Enclosure"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Remains"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Water"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Message"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Darkness"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Opening"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Weapon"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Entry"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Illumination"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Obstacle"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Craft"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Container"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Information"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Grave"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Equipment"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Shelter"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Denizen"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Environment"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Material"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Resource"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Corruption"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Death"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Function"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Power"
                }
              ]
            }
         }
        },
        site_name: {
          oracles: {
            format: {
              name: 'format',
              roll_id: 'Format',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-25',
                  result: "[Description] [Place]"
                },
                {
                  type: 'Between',
                  roll: '26-50',
                  result: "[Place] of [Detail]"
                },
                {
                  type: 'Between',
                  roll: '51-70',
                  result: "[Place] of [Description] [Detail]"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "[Place] of [Namesake's] [Detail]"
                },
                {
                  type: 'Between',
                  roll: '81-85',
                  result: "[Namesake's] [Place]"
                },
                {
                  type: 'Between',
                  roll: '86-95',
                  result: "[Description] [Place] of [Namesake]"
                },
                {
                  type: 'Between',
                  roll: '96-100',
                  result: "[Place] of [Namesake]"
                },
              ]
            },
            description: {
              name: 'description',
              roll_id: 'Description',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Deep"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Tainted"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Grey"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Forgotten"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Flooded"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Forbidden"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Barren"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Lost"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Cursed"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Fell"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Sunken"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Nightmare"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Infernal"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Dark"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Bloodstained"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Haunted"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "White"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Shrouded"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Wasted"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Grim"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Endless"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Crumbling"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Undying"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Bloodied"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Forsaken"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Silent"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Blighted"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Iron"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Frozen"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Abyssal"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Crimson"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Silver"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Desecrated"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Ashen"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Elder"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Scorched"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Unknown"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Scarred"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Broken"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Chaotic"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Black"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Hidden"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Sundered"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Shattered"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Dreaded"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Secret"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "High"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Sacred"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Fallen"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Ruined"
                }
              ]
            },
            detail: {
              name: 'detail',
              roll_id: 'Detail',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Blight"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Strife"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Nightfall"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Fury"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Terror"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Truth"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Spring"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Sanctuary"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Bone"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Specters"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Daybreak"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Doom"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Treachery"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Blood"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "War"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Torment"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Iron"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Silence"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Mist"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Isolation"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Runes"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Rot"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Corruption"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Prophecy"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Fate"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Twilight"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Power"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Darkness"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Gloom"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Storms"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Hope"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Lament"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Frost"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Souls"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Winter"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Sadness"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Desolation"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Bane"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Lies"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Ash"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Banishment"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Shadow"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Madness"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Stone"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Secrets"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Despair"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Blades"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Dread"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Light"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Wrath"
                }
              ]
            },
            namesake: {
              name: 'namesake',
              roll_id: 'Namesake',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Breckon"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Issara"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Milenna"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Thorval"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Khulan"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Aurvang"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Kalida"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Keeara"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Andor"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Zakaria"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Willa"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Etana"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Valgard"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Kenrick"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Wyland"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Sidura"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Svala"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Kajir"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Saiven"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Callwen"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Zhan"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Solana"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Ildar"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Keelan"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Thrain"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Kynan"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Jadina"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Radek"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Wulan"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Garion"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Eysa"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Kolor"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Katarra"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Dain"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Farina"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Yala"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Kodroth"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Morien"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Akida"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Haldorr"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Nyrad"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Edda"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Jorund"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Morraine"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Lindar"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Sithra"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Torgan"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Arnorr"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Thyri"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Erisia"
                }
              ]
            },
            place: {
              name: 'place',
              roll_id: 'Place',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-6',
                  result: "Barrow",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-6',
                      result: "Sepulcher"
                    },
                    {
                      type: 'Between',
                      roll: '17-32',
                      result: "Grave"
                    },
                    {
                      type: 'Between',
                      roll: '33-49',
                      result: "Crypt"
                    },
                    {
                      type: 'Between',
                      roll: '50-66',
                      result: "Mound"
                    },
                    {
                      type: 'Between',
                      roll: '67-83',
                      result: "Tomb"
                    },
                    {
                      type: 'Between',
                      roll: '84-100',
                      result: "Barrow"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '7-18',
                  result: "Cavern",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-10',
                      result: "Abyss"
                    },
                    {
                      type: 'Between',
                      roll: '11-20',
                      result: "Caverns"
                    },
                    {
                      type: 'Between',
                      roll: '21-30',
                      result: "Caves"
                    },
                    {
                      type: 'Between',
                      roll: '31-40',
                      result: "Chasm"
                    },
                    {
                      type: 'Between',
                      roll: '41-50',
                      result: "Depths"
                    },
                    {
                      type: 'Between',
                      roll: '51-60',
                      result: "Hollow"
                    },
                    {
                      type: 'Between',
                      roll: '61-70',
                      result: "Lair"
                    },
                    {
                      type: 'Between',
                      roll: '71-80',
                      result: "Rift"
                    },
                    {
                      type: 'Between',
                      roll: '81-90',
                      result: "Tunnels"
                    },
                    {
                      type: 'Between',
                      roll: '91-100',
                      result: "Warren"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '19-28',
                  result: "Frozen Cavern",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-10',
                      result: "Abyss"
                    },
                    {
                      type: 'Between',
                      roll: '11-20',
                      result: "Caverns"
                    },
                    {
                      type: 'Between',
                      roll: '21-30',
                      result: "Caves"
                    },
                    {
                      type: 'Between',
                      roll: '31-40',
                      result: "Chasm"
                    },
                    {
                      type: 'Between',
                      roll: '41-50',
                      result: "Depths"
                    },
                    {
                      type: 'Between',
                      roll: '51-60',
                      result: "Hollow"
                    },
                    {
                      type: 'Between',
                      roll: '61-70',
                      result: "Lair"
                    },
                    {
                      type: 'Between',
                      roll: '71-80',
                      result: "Rift"
                    },
                    {
                      type: 'Between',
                      roll: '81-90',
                      result: "Tunnels"
                    },
                    {
                      type: 'Between',
                      roll: '91-100',
                      result: "Warren"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Icereach",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-16',
                      result: "Icemark"
                    },
                    {
                      type: 'Between',
                      roll: '17-32',
                      result: "Wintertide"
                    },
                    {
                      type: 'Between',
                      roll: '33-49',
                      result: "Reach"
                    },
                    {
                      type: 'Between',
                      roll: '50-66',
                      result: "Waste"
                    },
                    {
                      type: 'Between',
                      roll: '67-83',
                      result: "Expanse"
                    },
                    {
                      type: 'Between',
                      roll: '84-100',
                      result: "Barrens"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '33-38',
                  result: "Mine",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-16',
                      result: "Lode"
                    },
                    {
                      type: 'Between',
                      roll: '17-32',
                      result: "Dig"
                    },
                    {
                      type: 'Between',
                      roll: '33-49',
                      result: "Forge"
                    },
                    {
                      type: 'Between',
                      roll: '50-66',
                      result: "Mine"
                    },
                    {
                      type: 'Between',
                      roll: '67-83',
                      result: "Tunnels"
                    },
                    {
                      type: 'Between',
                      roll: '84-100',
                      result: "Cut"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '39-48',
                  result: "Pass",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-10',
                      result: "Cliffs"
                    },
                    {
                      type: 'Between',
                      roll: '11-20',
                      result: "Crag"
                    },
                    {
                      type: 'Between',
                      roll: '21-30',
                      result: "Cut"
                    },
                    {
                      type: 'Between',
                      roll: '31-40',
                      result: "Gap"
                    },
                    {
                      type: 'Between',
                      roll: '41-50',
                      result: "Gorge"
                    },
                    {
                      type: 'Between',
                      roll: '51-60',
                      result: "Heights"
                    },
                    {
                      type: 'Between',
                      roll: '61-70',
                      result: "Highlands"
                    },
                    {
                      type: 'Between',
                      roll: '71-80',
                      result: "Pass"
                    },
                    {
                      type: 'Between',
                      roll: '81-90',
                      result: "Reach"
                    },
                    {
                      type: 'Between',
                      roll: '91-100',
                      result: "Ridge"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '49-58',
                  result: "Ruin",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-10',
                      result: "Citadel"
                    },
                    {
                      type: 'Between',
                      roll: '11-20',
                      result: "Enclave"
                    },
                    {
                      type: 'Between',
                      roll: '21-30',
                      result: "Fortress"
                    },
                    {
                      type: 'Between',
                      roll: '31-40',
                      result: "Hall"
                    },
                    {
                      type: 'Between',
                      roll: '41-50',
                      result: "Keep"
                    },
                    {
                      type: 'Between',
                      roll: '51-60',
                      result: "Sanctuary"
                    },
                    {
                      type: 'Between',
                      roll: '61-70',
                      result: "Sanctum"
                    },
                    {
                      type: 'Between',
                      roll: '71-80',
                      result: "Spire"
                    },
                    {
                      type: 'Between',
                      roll: '81-90',
                      result: "Temple"
                    },
                    {
                      type: 'Between',
                      roll: '91-100',
                      result: "Tower"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '59-68',
                  result: "Sea Cave",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-16',
                      result: "Caves"
                    },
                    {
                      type: 'Between',
                      roll: '17-32',
                      result: "Channel"
                    },
                    {
                      type: 'Between',
                      roll: '33-49',
                      result: "Cove"
                    },
                    {
                      type: 'Between',
                      roll: '50-66',
                      result: "Hollow"
                    },
                    {
                      type: 'Between',
                      roll: '67-83',
                      result: "Pools"
                    },
                    {
                      type: 'Between',
                      roll: '84-100',
                      result: "Gouge"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '69-78',
                  result: "Shadowfen",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-10',
                      result: "Bog"
                    },
                    {
                      type: 'Between',
                      roll: '11-20',
                      result: "Fen"
                    },
                    {
                      type: 'Between',
                      roll: '21-30',
                      result: "Lowland"
                    },
                    {
                      type: 'Between',
                      roll: '31-40',
                      result: "Marsh"
                    },
                    {
                      type: 'Between',
                      roll: '41-50',
                      result: "Mire"
                    },
                    {
                      type: 'Between',
                      roll: '51-60',
                      result: "Morass"
                    },
                    {
                      type: 'Between',
                      roll: '61-70',
                      result: "Quagmire"
                    },
                    {
                      type: 'Between',
                      roll: '71-80',
                      result: "Floodlands"
                    },
                    {
                      type: 'Between',
                      roll: '81-90',
                      result: "Slough"
                    },
                    {
                      type: 'Between',
                      roll: '91-100',
                      result: "Wetlands"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '79-83',
                  result: "Stronghold",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-10',
                      result: "Bastion"
                    },
                    {
                      type: 'Between',
                      roll: '11-20',
                      result: "Citadel"
                    },
                    {
                      type: 'Between',
                      roll: '21-30',
                      result: "Fortress"
                    },
                    {
                      type: 'Between',
                      roll: '31-40',
                      result: "Garrison"
                    },
                    {
                      type: 'Between',
                      roll: '41-50',
                      result: "Haven"
                    },
                    {
                      type: 'Between',
                      roll: '51-60',
                      result: "Keep"
                    },
                    {
                      type: 'Between',
                      roll: '61-70',
                      result: "Outpost"
                    },
                    {
                      type: 'Between',
                      roll: '71-80',
                      result: "Refuge"
                    },
                    {
                      type: 'Between',
                      roll: '81-90',
                      result: "Sanctuary"
                    },
                    {
                      type: 'Between',
                      roll: '91-100',
                      result: "Watch"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '84-95',
                  result: "Tanglewood",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-11',
                      result: "Weald"
                    },
                    {
                      type: 'Between',
                      roll: '12-23',
                      result: "Tangle"
                    },
                    {
                      type: 'Between',
                      roll: '24-35',
                      result: "Bramble"
                    },
                    {
                      type: 'Between',
                      roll: '36-48',
                      result: "Briar"
                    },
                    {
                      type: 'Between',
                      roll: '49-61',
                      result: "Thicket"
                    },
                    {
                      type: 'Between',
                      roll: '62-74',
                      result: "Forest"
                    },
                    {
                      type: 'Between',
                      roll: '75-87',
                      result: "Wilds"
                    },
                    {
                      type: 'Between',
                      roll: '88-100',
                      result: "Wood"
                    }
                  ]
                },
                {
                  type: 'Between',
                  roll: '96-100',
                  result: "Underkeep",
                  nested_oracle: [
                    {
                      type: 'Between',
                      roll: '1-10',
                      result: "Catacomb"
                    },
                    {
                      type: 'Between',
                      roll: '11-20',
                      result: "Chambers"
                    },
                    {
                      type: 'Between',
                      roll: '21-30',
                      result: "Den"
                    },
                    {
                      type: 'Between',
                      roll: '31-40',
                      result: "Hall"
                    },
                    {
                      type: 'Between',
                      roll: '41-50',
                      result: "Labyrinth"
                    },
                    {
                      type: 'Between',
                      roll: '51-60',
                      result: "Maze"
                    },
                    {
                      type: 'Between',
                      roll: '61-70',
                      result: "Pit"
                    },
                    {
                      type: 'Between',
                      roll: '71-80',
                      result: "Sanctum"
                    },
                    {
                      type: 'Between',
                      roll: '81-90',
                      result: "Underkeep"
                    },
                    {
                      type: 'Between',
                      roll: '91-100',
                      result: "Vault"
                    }
                  ]
                }
              ]
            }
          }
        },
        site_nature: {
          oracles: {
            theme: {
              name: 'theme',
              roll_id: 'Theme',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-11',
                  result: "Ancient"
                },
                {
                  type: 'Between',
                  roll: '12-23',
                  result: "Corrupted"
                },
                {
                  type: 'Between',
                  roll: '24-35',
                  result: "Fortified"
                },
                {
                  type: 'Between',
                  roll: '36-48',
                  result: "Hallowed"
                },
                {
                  type: 'Between',
                  roll: '49-61',
                  result: "Haunted"
                },
                {
                  type: 'Between',
                  roll: '62-74',
                  result: "Infested"
                },
                {
                  type: 'Between',
                  roll: '75-87',
                  result: "Ravaged"
                },
                {
                  type: 'Between',
                  roll: '88-100',
                  result: "Wild"
                }
              ]
            },
            domain: {
              name: 'domain',
              roll_id: 'Domain',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-6',
                  result: "Barrow"
                },
                {
                  type: 'Between',
                  roll: '7-18',
                  result: "Cavern"
                },
                {
                  type: 'Between',
                  roll: '19-28',
                  result: "Frozen Cavern"
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Icereach"
                },
                {
                  type: 'Between',
                  roll: '33-38',
                  result: "Mine"
                },
                {
                  type: 'Between',
                  roll: '39-48',
                  result: "Pass"
                },
                {
                  type: 'Between',
                  roll: '49-58',
                  result: "Ruin"
                },
                {
                  type: 'Between',
                  roll: '59-68',
                  result: "Sea Cave"
                },
                {
                  type: 'Between',
                  roll: '69-78',
                  result: "Shadowfen"
                },
                {
                  type: 'Between',
                  roll: '79-83',
                  result: "Stronghold"
                },
                {
                  type: 'Between',
                  roll: '84-95',
                  result: "Tanglewood"
                },
                {
                  type: 'Between',
                  roll: '96-100',
                  result: "Underkeep"
                }
              ]
            }
          }
        },
        monstrosity: {
          oracles: {
            size: {
              name: 'size',
              roll_id: 'Size',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-5',
                  result: "Tiny (rodent-sized)"
                },
                {
                  type: 'Between',
                  roll: '6-30',
                  result: "Small (hound-sized)"
                },
                {
                  type: 'Between',
                  roll: '31-65',
                  result: "Medium (person-sized)"
                },
                {
                  type: 'Between',
                  roll: '66-94',
                  result: "Large (giant-sized)"
                },
                {
                  type: 'Between',
                  roll: '95-99',
                  result: "Huge (whale-sized)"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Titanic (incomprehensible)"
                }
              ]
            },
            primary_form: {
              name: 'primary_form',
              roll_id: 'PrimaryForm',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-15',
                  result: "Beast / mammal"
                },
                {
                  type: 'Between',
                  roll: '16-25',
                  result: "Humanoid"
                },
                {
                  type: 'Between',
                  roll: '26-31',
                  result: "Bird"
                },
                {
                  type: 'Between',
                  roll: '32-37',
                  result: "Spider"
                },
                {
                  type: 'Between',
                  roll: '38-43',
                  result: "Snake"
                },
                {
                  type: 'Between',
                  roll: '44-49',
                  result: "Worm / slug"
                },
                {
                  type: 'Between',
                  roll: '50-55',
                  result: "Lizard"
                },
                {
                  type: 'Between',
                  roll: '56-61',
                  result: "Insect"
                },
                {
                  type: 'Between',
                  roll: '62-66',
                  result: "Amorphous"
                },
                {
                  type: 'Between',
                  roll: '67-69',
                  result: "Crustacean"
                },
                {
                  type: 'Between',
                  roll: '70-71',
                  result: "Fish"
                },
                {
                  type: 'Between',
                  roll: '72-73',
                  result: "Octopoid"
                },
                {
                  type: 'Between',
                  roll: '74-75',
                  result: "Amphibian"
                },
                {
                  type: 'Between',
                  roll: '76-77',
                  result: "Plant"
                },
                {
                  type: 'Total',
                  roll: '78',
                  result: "Incorporeal"
                },
                {
                  type: 'Total',
                  roll: '79',
                  result: "Mineral"
                },
                {
                  type: 'Total',
                  roll: '80',
                  result: "Elemental"
                },
                {
                  type: 'Between',
                  roll: '81-100',
                  result: "Hybrid (roll twice)"
                }
              ]
            },
            characteristics: {
              name: 'characteristics',
              roll_id: 'Characteristics',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-5',
                  result: "Extra limbs"
                },
                {
                  type: 'Between',
                  roll: '6-10',
                  result: "Fangs / rows of sharp teeth"
                },
                {
                  type: 'Between',
                  roll: '11-15',
                  result: "Claws / talons"
                },
                {
                  type: 'Between',
                  roll: '16-20',
                  result: "Strange color / markings"
                },
                {
                  type: 'Between',
                  roll: '21-25',
                  result: "Horns / tusks"
                },
                {
                  type: 'Between',
                  roll: '26-30',
                  result: "Oversized mouth"
                },
                {
                  type: 'Between',
                  roll: '31-35',
                  result: "Spikes / spines"
                },
                {
                  type: 'Between',
                  roll: '36-40',
                  result: "Tail"
                },
                {
                  type: 'Between',
                  roll: '41-45',
                  result: "Multi-segmented body"
                },
                {
                  type: 'Between',
                  roll: '46-50',
                  result: "Wings"
                },
                {
                  type: 'Between',
                  roll: '51-54',
                  result: "Stinger / barbs"
                },
                {
                  type: 'Between',
                  roll: '55-58',
                  result: "Many-eyed"
                },
                {
                  type: 'Between',
                  roll: '59-62',
                  result: "Distinctive sound"
                },
                {
                  type: 'Between',
                  roll: '63-66',
                  result: "Tentacles / tendrils"
                },
                {
                  type: 'Between',
                  roll: '67-69',
                  result: "Mandibles / pincers"
                },
                {
                  type: 'Between',
                  roll: '70-72',
                  result: "Luminescent"
                },
                {
                  type: 'Between',
                  roll: '73-75',
                  result: "Antennae / sensory organs"
                },
                {
                  type: 'Between',
                  roll: '76-78',
                  result: "Proboscis / inner jaw"
                },
                {
                  type: 'Between',
                  roll: '79-81',
                  result: "Exoskeleton / shell"
                },
                {
                  type: 'Between',
                  roll: '82-84',
                  result: "Bony protuberances"
                },
                {
                  type: 'Between',
                  roll: '85-87',
                  result: "Corrupted flesh"
                },
                {
                  type: 'Between',
                  roll: '88-90',
                  result: "Semi-transparent"
                },
                {
                  type: 'Between',
                  roll: '91-93',
                  result: "Scarred / injured"
                },
                {
                  type: 'Between',
                  roll: '94-95',
                  result: "Egg sac / carried offspring"
                },
                {
                  type: 'Between',
                  roll: '96-97',
                  result: "Rotting / skeletal"
                },
                {
                  type: 'Total',
                  roll: '98',
                  result: "Mummified / desiccated"
                },
                {
                  type: 'Total',
                  roll: '99',
                  result: "Multi-headed"
                },
                {
                  type: 'Between',
                  roll: '100',
                  result: "Etched with mystic runes"
                }
              ]
            },
            abilities: {
              name: 'abilities',
              roll_id: 'Abilities',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-4',
                  result: "Keen senses"
                },
                {
                  type: 'Between',
                  roll: '5-8',
                  result: "Intimidating vocalization"
                },
                {
                  type: 'Between',
                  roll: '9-12',
                  result: "Climber"
                },
                {
                  type: 'Between',
                  roll: '13-16',
                  result: "Intelligent"
                },
                {
                  type: 'Between',
                  roll: '17-20',
                  result: "Swift"
                },
                {
                  type: 'Between',
                  roll: '21-24',
                  result: "Powerful bite"
                },
                {
                  type: 'Between',
                  roll: '25-28',
                  result: "Stealthy / ambusher"
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Horrid visage"
                },
                {
                  type: 'Between',
                  roll: '33-36',
                  result: "Strong"
                },
                {
                  type: 'Between',
                  roll: '37-40',
                  result: "Camouflaged"
                },
                {
                  type: 'Between',
                  roll: '41-43',
                  result: "Flier / glider"
                },
                {
                  type: 'Between',
                  roll: '44-46',
                  result: "Poisonous"
                },
                {
                  type: 'Between',
                  roll: '47-49',
                  result: "Semiaquatic / swimmer"
                },
                {
                  type: 'Between',
                  roll: '50-52',
                  result: "Grappler / entangler"
                },
                {
                  type: 'Between',
                  roll: '53-55',
                  result: "Leaper"
                },
                {
                  type: 'Between',
                  roll: '56-58',
                  result: "Crusher / constrictor"
                },
                {
                  type: 'Between',
                  roll: '59-61',
                  result: "Armored"
                },
                {
                  type: 'Between',
                  roll: '62-64',
                  result: "Burrower"
                },
                {
                  type: 'Between',
                  roll: '65-67',
                  result: "Noxious smell"
                },
                {
                  type: 'Between',
                  roll: '68-69',
                  result: "Trap-setter"
                },
                {
                  type: 'Between',
                  roll: '70-71',
                  result: "Parasitic"
                },
                {
                  type: 'Between',
                  roll: '72-73',
                  result: "Vibration sense"
                },
                {
                  type: 'Between',
                  roll: '74-75',
                  result: "Breath weapon / toxic spew"
                },
                {
                  type: 'Between',
                  roll: '76-77',
                  result: "Mimicry"
                },
                {
                  type: 'Between',
                  roll: '78-79',
                  result: "Shapeshifting"
                },
                {
                  type: 'Between',
                  roll: '80-81',
                  result: "Control lesser creatures"
                },
                {
                  type: 'Between',
                  roll: '82-83',
                  result: "Echolocation"
                },
                {
                  type: 'Between',
                  roll: '84-85',
                  result: "Electric shock"
                },
                {
                  type: 'Between',
                  roll: '86-87',
                  result: "Acidic"
                },
                {
                  type: 'Between',
                  roll: '88-89',
                  result: "Symbiotic"
                },
                {
                  type: 'Between',
                  roll: '90-91',
                  result: "Shoot projectiles"
                },
                {
                  type: 'Total',
                  roll: '92',
                  result: "Paralyzing"
                },
                {
                  type: 'Total',
                  roll: '93',
                  result: "Immune to iron"
                },
                {
                  type: 'Total',
                  roll: '94',
                  result: "Feels no pain"
                },
                {
                  type: 'Total',
                  roll: '95',
                  result: "Enact rituals"
                },
                {
                  type: 'Total',
                  roll: '96',
                  result: "Create illusions"
                },
                {
                  type: 'Total',
                  roll: '97',
                  result: "Mind control / telepathy"
                },
                {
                  type: 'Total',
                  roll: '98',
                  result: "Move between realities"
                },
                {
                  type: 'Total',
                  roll: '99',
                  result: "Wield weapons"
                },
                {
                  type: 'Total',
                  roll: '100',
                  result: "Control elements"
                }
              ]
            }
          }
        },
        trap: {
          oracles: {
            event: {
              name: 'event',
              roll_id: 'Event',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-4',
                  result: "Block"
                },
                {
                  type: 'Between',
                  roll: '5-8',
                  result: "Create"
                },
                {
                  type: 'Between',
                  roll: '9-12',
                  result: "Break"
                },
                {
                  type: 'Between',
                  roll: '13-16',
                  result: "Puncture"
                },
                {
                  type: 'Between',
                  roll: '17-20',
                  result: "Entangle"
                },
                {
                  type: 'Between',
                  roll: '21-24',
                  result: "Enclose"
                },
                {
                  type: 'Between',
                  roll: '25-28',
                  result: "Ambush"
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Snare"
                },
                {
                  type: 'Between',
                  roll: '33-36',
                  result: "Change"
                },
                {
                  type: 'Between',
                  roll: '37-40',
                  result: "Imitate"
                },
                {
                  type: 'Between',
                  roll: '41-44',
                  result: "Crush"
                },
                {
                  type: 'Between',
                  roll: '45-48',
                  result: "Drop"
                },
                {
                  type: 'Between',
                  roll: '49-52',
                  result: "Conceal"
                },
                {
                  type: 'Between',
                  roll: '53-56',
                  result: "Lure"
                },
                {
                  type: 'Between',
                  roll: '57-60',
                  result: "Release"
                },
                {
                  type: 'Between',
                  roll: '61-64',
                  result: "Obscure"
                },
                {
                  type: 'Between',
                  roll: '65-68',
                  result: "Cut"
                },
                {
                  type: 'Between',
                  roll: '69-72',
                  result: "Smother"
                },
                {
                  type: 'Between',
                  roll: '73-76',
                  result: "Collapse"
                },
                {
                  type: 'Between',
                  roll: '77-80',
                  result: "Summon"
                },
                {
                  type: 'Between',
                  roll: '81-84',
                  result: "Move"
                },
                {
                  type: 'Between',
                  roll: '85-88',
                  result: "Surprise"
                },
                {
                  type: 'Between',
                  roll: '89-92',
                  result: "Divert"
                },
                {
                  type: 'Between',
                  roll: '93-96',
                  result: "Attack"
                },
                {
                  type: 'Between',
                  roll: '97-100',
                  result: "Trigger"
                }
              ]
            },
            component: {
              name: 'component',
              roll_id: 'Component',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-4',
                  result: "Pit"
                },
                {
                  type: 'Between',
                  roll: '5-8',
                  result: "Water"
                },
                {
                  type: 'Between',
                  roll: '9-12',
                  result: "Fire"
                },
                {
                  type: 'Between',
                  roll: '13-16',
                  result: "Projectile"
                },
                {
                  type: 'Between',
                  roll: '17-20',
                  result: "Passage"
                },
                {
                  type: 'Between',
                  roll: '21-24',
                  result: "Fall"
                },
                {
                  type: 'Between',
                  roll: '25-28',
                  result: "Debris"
                },
                {
                  type: 'Between',
                  roll: '29-32',
                  result: "Fear"
                },
                {
                  type: 'Between',
                  roll: '33-36',
                  result: "Alarm"
                },
                {
                  type: 'Between',
                  roll: '37-40',
                  result: "Trigger"
                },
                {
                  type: 'Between',
                  roll: '41-44',
                  result: "Cold"
                },
                {
                  type: 'Between',
                  roll: '45-48',
                  result: "Weapon"
                },
                {
                  type: 'Between',
                  roll: '49-52',
                  result: "Darkness"
                },
                {
                  type: 'Between',
                  roll: '53-56',
                  result: "Decay"
                },
                {
                  type: 'Between',
                  roll: '57-60',
                  result: "Path"
                },
                {
                  type: 'Between',
                  roll: '61-64',
                  result: "Stone"
                },
                {
                  type: 'Between',
                  roll: '65-68',
                  result: "Terrain"
                },
                {
                  type: 'Between',
                  roll: '69-72',
                  result: "Poison"
                },
                {
                  type: 'Between',
                  roll: '73-76',
                  result: "Barrier"
                },
                {
                  type: 'Between',
                  roll: '77-80',
                  result: "Overhead"
                },
                {
                  type: 'Between',
                  roll: '81-84',
                  result: "Magic"
                },
                {
                  type: 'Between',
                  roll: '85-88',
                  result: "Toxin"
                },
                {
                  type: 'Between',
                  roll: '89-92',
                  result: "Earth"
                },
                {
                  type: 'Between',
                  roll: '93-96',
                  result: "Light"
                },
                {
                  type: 'Between',
                  roll: '97-100',
                  result: "Denizen"
                }
              ]
            }
          }
        },
        combat_event: {
          oracles: {
            method: {
              name: 'method',
              roll_id: 'Method',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Defy"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Break"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Trick"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Evade"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Protect"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Overwhelm"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Persevere"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Assist"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Await"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Abort"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Block"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Collide"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Focus"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Advance"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Breach"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Endure"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Assault"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Charge"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Escalate"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Sunder"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Shatter"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Aim"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Stagger"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Counter"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Seize"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Impact"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Entangle"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Hold"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Deflect"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Drop"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Lose"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Sweep"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Secure"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Cover"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Withdraw"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Clash"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Amplify"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Batter"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Feint"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Shove"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Embed"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Affect"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Probe"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Force"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Intensify"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Distract"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Challenge"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Brawl"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Coordinate"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Overrun"
                }
              ]
            },
            target: {
              name: 'target',
              roll_id: 'Target',
              dice: 'd100',
              columns: '2',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Control"
                },
                {
                  type: 'Between',
                  roll: '3-4',
                  result: "Defense"
                },
                {
                  type: 'Between',
                  roll: '5-6',
                  result: "Limbs"
                },
                {
                  type: 'Between',
                  roll: '7-8',
                  result: "Focus"
                },
                {
                  type: 'Between',
                  roll: '9-10',
                  result: "Advantage"
                },
                {
                  type: 'Between',
                  roll: '11-12',
                  result: "Range"
                },
                {
                  type: 'Between',
                  roll: '13-14',
                  result: "Stress"
                },
                {
                  type: 'Between',
                  roll: '15-16',
                  result: "Sense"
                },
                {
                  type: 'Between',
                  roll: '17-18',
                  result: "Weakness"
                },
                {
                  type: 'Between',
                  roll: '19-20',
                  result: "Opening"
                },
                {
                  type: 'Between',
                  roll: '21-22',
                  result: "Fear"
                },
                {
                  type: 'Between',
                  roll: '23-24',
                  result: "Instinct"
                },
                {
                  type: 'Between',
                  roll: '25-26',
                  result: "Footing"
                },
                {
                  type: 'Between',
                  roll: '27-28',
                  result: "Maneuver"
                },
                {
                  type: 'Between',
                  roll: '29-30',
                  result: "Reach"
                },
                {
                  type: 'Between',
                  roll: '31-32',
                  result: "Harm"
                },
                {
                  type: 'Between',
                  roll: '33-34',
                  result: "Finesse"
                },
                {
                  type: 'Between',
                  roll: '35-36',
                  result: "Weapon"
                },
                {
                  type: 'Between',
                  roll: '37-38',
                  result: "Environment"
                },
                {
                  type: 'Between',
                  roll: '39-40',
                  result: "Technique"
                },
                {
                  type: 'Between',
                  roll: '41-42',
                  result: "Surprise"
                },
                {
                  type: 'Between',
                  roll: '43-44',
                  result: "Pride"
                },
                {
                  type: 'Between',
                  roll: '45-46',
                  result: "Wound"
                },
                {
                  type: 'Between',
                  roll: '47-48',
                  result: "Precision"
                },
                {
                  type: 'Between',
                  roll: '49-50',
                  result: "Ally"
                },
                {
                  type: 'Between',
                  roll: '51-52',
                  result: "Ground"
                },
                {
                  type: 'Between',
                  roll: '53-54',
                  result: "Courage"
                },
                {
                  type: 'Between',
                  roll: '55-56',
                  result: "Companion"
                },
                {
                  type: 'Between',
                  roll: '57-58',
                  result: "Object"
                },
                {
                  type: 'Between',
                  roll: '59-60',
                  result: "Momentum"
                },
                {
                  type: 'Between',
                  roll: '61-62',
                  result: "Speed"
                },
                {
                  type: 'Between',
                  roll: '63-64',
                  result: "Strength"
                },
                {
                  type: 'Between',
                  roll: '65-66',
                  result: "Supply"
                },
                {
                  type: 'Between',
                  roll: '67-68',
                  result: "Terrain"
                },
                {
                  type: 'Between',
                  roll: '69-70',
                  result: "Armor"
                },
                {
                  type: 'Between',
                  roll: '71-72',
                  result: "Skill"
                },
                {
                  type: 'Between',
                  roll: '73-74',
                  result: "Body"
                },
                {
                  type: 'Between',
                  roll: '75-76',
                  result: "Protection"
                },
                {
                  type: 'Between',
                  roll: '77-78',
                  result: "Resolve"
                },
                {
                  type: 'Between',
                  roll: '79-80',
                  result: "Ferocity"
                },
                {
                  type: 'Between',
                  roll: '81-82',
                  result: "Shield"
                },
                {
                  type: 'Between',
                  roll: '83-84',
                  result: "Ammo"
                },
                {
                  type: 'Between',
                  roll: '85-86',
                  result: "Anger"
                },
                {
                  type: 'Between',
                  roll: '87-88',
                  result: "Opportunity"
                },
                {
                  type: 'Between',
                  roll: '89-90',
                  result: "Balance"
                },
                {
                  type: 'Between',
                  roll: '91-92',
                  result: "Position"
                },
                {
                  type: 'Between',
                  roll: '93-94',
                  result: "Barrier"
                },
                {
                  type: 'Between',
                  roll: '95-96',
                  result: "Strategy"
                },
                {
                  type: 'Between',
                  roll: '97-98',
                  result: "Grasp"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Power"
                }
              ]
            }
          }
        },
        threat: {
          oracles: {
            category: {
              name: 'category',
              roll_id: 'Category',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Burgeoning Conflict"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Cursed Site"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Environmental Calamity"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Malignant Plague"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Rampaging Creature"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Ravaging Horde"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Scheming Leader"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Power-Hungry Mystic"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Zealous Cult"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Roll Twice"
                }
              ]
            },
            burgeoning_conflict: {
              name: 'burgeoning_conflict',
              roll_id: 'BurgeoningConflict',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Allow warmongers to gain influence"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Break a treaty"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Force a hasty decision"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Deepen suspicions"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Trigger a confrontation"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Subvert a potential accord"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Isolate the antagonists"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Draw new battle lines"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Reveal an unexpected aspect of the dispute"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Introduce a new person or faction to complicate the situation"
                }
              ]
            },
            cursed_site: {
              name: 'cursed_site',
              roll_id: 'CursedSite',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Unleash a creature or being"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Lure the unwary into its depths"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Offer promises of power"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Reveal a new aspect of its cursed history"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Expand its malignancy to surrounding lands"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Leave its mark on an inhabitant or visitor"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Reveal hidden depths"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Ensnare an important person or object"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Corrupt the environment"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Transform its nature"
                }
              ]
            },
            environmental_calamity: {
              name: 'environmental_calamity',
              roll_id: 'EnvironmentalCalamity',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Devastate a place"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Block a path"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Threaten a community with imminent destruction"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Manifest unexpected effects"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Expand in scope or intensity"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Allow someone to take advantage"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Deprive of resources"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Isolate an important person or community"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Force refugees into hostile lands"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Disrupt natural ecosystems"
                }
              ]
            },
            malignant_plague: {
              name: 'malignant_plague',
              roll_id: 'MalignantPlague',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Manifest new symptoms or effects"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Infect someone important"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Expand to new territory or communities"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Allow someone to take advantage"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Allow someone to take the blame"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Create panic or disorder"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Force a horrible decision"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Lure into complacency"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Reveal the root of the sickness"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Undermine a potential cure"
                }
              ]
            },
            rampaging_creature: {
              name: 'rampaging_creature',
              roll_id: 'RampagingCreature',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Reveal a new aspect of its nature or abilities"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Expand its territory"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Make a sudden and brutal attack"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Control or influence lesser creatures"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Create confusion or strife"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Leave foreboding signs"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Lure the unwary"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Imperil an event"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Assert control over a location"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Threaten resources"
                }
              ]
            },
            ravaging_horde: {
              name: 'ravaging_horde',
              roll_id: 'RavagingHorde',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Overrun defenses"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Gather resources"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Attack a location"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Expand forces"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Appoint or reveal a leader"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Send forth a champion"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Create a diversion"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Undermine an opposing force from within"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Cut off supplies or reinforcements"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Employ a new weapon"
                }
              ]
            },
            scheming_leader: {
              name: 'scheming_leader',
              roll_id: 'SchemingLeader',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Defeat an enemy"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Form a new alliance"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Usurp or undermine another leader"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Force the loyalty of a community or important person"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Enact a new law or tradition"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Rescind an old law or tradition"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Reveal a true intention"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Unravel an existing alliance"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Incite conflict"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Use an unexpected capability or asset"
                }
              ]
            },
            power_hungry_mystic: {
              name: 'power_hungry_mystic',
              roll_id: 'PowerHungryMystic',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Gain hidden knowledge"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Assault an enemy with magic"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Despoil a place through magic"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Forge a bond with ancient forces"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Create magical wards or protections"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Obtain a powerful artifact"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Tempt with power or secrets"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Recruit a follower or ally"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Sacrifice something in exchange for greater power"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Use magic to trick or deceive"
                }
              ]
            },
            zealous_cult: {
              name: 'zealous_cult',
              roll_id: 'ZealousCult',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "Overtake a faction or community"
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "Unlock secrets to greater power"
                },
                {
                  type: 'Between',
                  roll: '21-30',
                  result: "Establish false credibility"
                },
                {
                  type: 'Between',
                  roll: '31-40',
                  result: "Appoint or reveal a leader"
                },
                {
                  type: 'Between',
                  roll: '41-50',
                  result: "Lure new members or establish alliances"
                },
                {
                  type: 'Between',
                  roll: '51-60',
                  result: "Subvert opposition through devious schemes"
                },
                {
                  type: 'Between',
                  roll: '61-70',
                  result: "Attack opposition directly"
                },
                {
                  type: 'Between',
                  roll: '71-80',
                  result: "Spread the word of its doctrine"
                },
                {
                  type: 'Between',
                  roll: '81-90',
                  result: "Reveal a dire prophecy"
                },
                {
                  type: 'Between',
                  roll: '91-100',
                  result: "Reveal its true nature or goal"
                }
              ]
            }
          }
        }
      },
      moves: {
        sites: {
          oracles: {
            find_an_opportunity: {
              name: 'find_an_opportunity',
              roll_id: 'findAnOpportunity',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-25',
                  result: "The terrain favors you, or you find a hidden path."
                },
                {
                  type: 'Between',
                  roll: '26-45',
                  result: "An aspect of the history or nature of this place is revealed."
                },
                {
                  type: 'Between',
                  roll: '46-57',
                  result: "You locate a secure area."
                },
                {
                  type: 'Between',
                  roll: '58-68',
                  result: "A clue offers insight or direction."
                },
                {
                  type: 'Between',
                  roll: '69-78',
                  result: "You get the drop on a denizen."
                },
                {
                  type: 'Between',
                  roll: '79-86',
                  result: "This area provides an opportunity to scavenge, forage, or hunt."
                },
                {
                  type: 'Between',
                  roll: '87-90',
                  result: "You locate an interesting or helpful object."
                },
                {
                  type: 'Between',
                  roll: '91-94',
                  result: "You are alerted to a potential threat."
                },
                {
                  type: 'Between',
                  roll: '95-98',
                  result: "You encounter a denizen who might support you."
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "You encounter a denizen in need of help."
                }
              ]
            },
            reveal_a_danger: {
              name: 'reveal_a_danger',
              roll_id: 'revealADanger',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-30',
                  result: "Check the theme card."
                },
                {
                  type: 'Between',
                  roll: '31-45',
                  result: "Check the domain card."
                },
                {
                  type: 'Between',
                  roll: '46-57',
                  result: "You encounter a hostile denizen."
                },
                {
                  type: 'Between',
                  roll: '58-68',
                  result: "You face an environmental or architectural hazard."
                },
                {
                  type: 'Between',
                  roll: '69-76',
                  result: "A discovery undermines or complicates your quest."
                },
                {
                  type: 'Between',
                  roll: '77-79',
                  result: "You confront a harrowing situation or sensation."
                },
                {
                  type: 'Between',
                  roll: '80-82',
                  result: "You face the consequences of an earlier choice or approach."
                },
                {
                  type: 'Between',
                  roll: '83-85',
                  result: "Your way is blocked or trapped."
                },
                {
                  type: 'Between',
                  roll: '86-88',
                  result: "A resource is diminished, broken, or lost."
                },
                {
                  type: 'Between',
                  roll: '89-91',
                  result: "You face a perplexing mystery or tough choice."
                },
                {
                  type: 'Between',
                  roll: '92-94',
                  result: "You lose your way or are delayed."
                },
                {
                  type: 'Between',
                  roll: '95-100',
                  result: "Roll twice more on this table. Both results occur. If they are the same result, make it worse."
                }
              ]
            },
            reveal_a_danger_alt: {
              name: 'reveal_a_danger_alt',
              roll_id: 'revealADangerAlt',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-22',
                  result: "You encounter a hostile denizen."
                },
                {
                  type: 'Between',
                  roll: '23-42',
                  result: "You face an environmental or architectural hazard."
                },
                {
                  type: 'Between',
                  roll: '43-58',
                  result: "A discovery undermines or complicates your quest."
                },
                {
                  type: 'Between',
                  roll: '59-64',
                  result: "You confront a harrowing situation or sensation."
                },
                {
                  type: 'Between',
                  roll: '65-70',
                  result: "You face the consequences of an earlier choice or approach."
                },
                {
                  type: 'Between',
                  roll: '71-76',
                  result: "The path is blocked or trapped."
                },
                {
                  type: 'Between',
                  roll: '77-82',
                  result: "A resource is diminished, broken, or lost."
                },
                {
                  type: 'Between',
                  roll: '83-88',
                  result: "You face a perplexing mystery or tough choice."
                },
                {
                  type: 'Between',
                  roll: '89-94',
                  result: "You lose your way or are delayed."
                },
                {
                  type: 'Between',
                  roll: '95-100',
                  result: "Roll twice more on this table. Both results occur. If they are the same result, make it worse."
                }
              ]
            }
          }
        },
        threat:{
          oracles: {
            advance_a_threat: {
              name: 'advance_a_threat',
              roll_id: 'AdvanceAThreat',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-30',
                  result: "The threat readies its next step, or a new danger looms. If you are in a position to prevent this development, you may attempt to do so. If you succeed, <em>Reach a Milestone.</em> Otherwise, mark menace."
                },
                {
                  type: 'Between',
                  roll: '31-70',
                  result: "The threat works subtly to advance toward its goal, or the danger escalates. Mark menace."
                },
                {
                  type: 'Between',
                  roll: '71-100',
                  result: "The threat makes a dramatic and immediate move, or a major event reveals new complications. Mark menace twice."
                },
              ]
            }
          }
        },
        suffer: {
          oracles: {
            endure_harm: {
              name: 'endure_harm',
              roll_id: 'EndureHarm',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "The harm is mortal."
                },
                {
                  type: 'Between',
                  roll: '11-20',
                  result: "You are dying. You need to Heal within an hour or two, or <em>Face Death</em>."
                },
                {
                  type: 'Between',
                  roll: '21-35',
                  result: "You are unconscious and out of action. If left alone, you come back to your senses in an hour or two. If you are vulnerable to a foe not inclined to show mercy, <em>Face Death</em>."
                },
                {
                  type: 'Between',
                  roll: '36-50',
                  result: "You are reeling and fighting to stay conscious. If you engage in any vigorous activity (such as running or fighting) before taking a breather for a few minutes, roll on this table again (before resolving the other move)."
                },
                {
                  type: 'Between',
                  roll: '51-100',
                  result: "You are battered but still standing."
                }
              ]
            },
            endure_stress: {
              name: 'endure_stress',
              roll_id: 'EndureStress',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-10',
                  result: "You are overwhelmed. <em>Face Desolation.</em>"
                },
                {
                  type: 'Between',
                  roll: '11-25',
                  result: "You give up. <em>Forsake Your Vow</em> (if possible, one relevant to your current crisis)."
                },
                {
                  type: 'Between',
                  roll: '26-50',
                  result: "You give in to a fear or compulsion, and act against your better instincts."
                },
                {
                  type: 'Between',
                  roll: '51-100',
                  result: "You persevere"
                }
              ]
            }
          }
        },
        fate: {
          oracles: {
            pay_the_price: {
              name: 'pay_the_price',
              roll_id: 'PayThePrice',
              dice: 'd100',
              columns: '1',
              rows: [
                {
                  type: 'Between',
                  roll: '1-2',
                  result: "Roll again and apply that result but make it worse. If you roll this result yet again, think of something dreadful that changes the course of your quest (<em>Ask the Oracle</em> if unsure) and make it happen."
                },
                {
                  type: 'Between',
                  roll: '3-5',
                  result: "A person or community you trusted loses faith in you, or acts against you."
                },
                {
                  type: 'Between',
                  roll: '6-9',
                  result: "A person or community you care about is exposed to danger."
                },
                {
                  type: 'Between',
                  roll: '10-16',
                  result: "You are separated from something or someone."
                },
                {
                  type: 'Between',
                  roll: '17-23',
                  result: "Your action has an unintended effect."
                },
                {
                  type: 'Between',
                  roll: '24-32',
                  result: "Something of value is lost or destroyed."
                },
                {
                  type: 'Between',
                  roll: '33-41',
                  result: "The current situation worsens."
                },
                {
                  type: 'Between',
                  roll: '42-50',
                  result: "A new danger or foe is revealed."
                },
                {
                  type: 'Between',
                  roll: '51-59',
                  result: "It causes a delay or puts you at a disadvantage."
                },
                {
                  type: 'Between',
                  roll: '60-68',
                  result: "It is harmful."
                },
                {
                  type: 'Between',
                  roll: '69-77',
                  result: "It is stressful."
                },
                {
                  type: 'Between',
                  roll: '78-85',
                  result: "A surprising development complicates your quest."
                },
                {
                  type: 'Between',
                  roll: '86-90',
                  result: "It wastes resources."
                },
                {
                  type: 'Between',
                  roll: '91-94',
                  result: "It forces you to act against your best intentions."
                },
                {
                  type: 'Between',
                  roll: '95-98',
                  result: "A friend, companion, or ally is put in harm’s way (or you are, if alone)"
                },
                {
                  type: 'Between',
                  roll: '99-100',
                  result: "Roll twice more on this table. Both results occur. If they are the same result, make  it worse."
                }
              ]
            }
          }
        }
      }
    }

